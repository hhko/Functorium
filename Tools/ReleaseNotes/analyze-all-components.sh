...
# Don't use set -e as some git commands may return non-zero for valid reasons
# set -e
...
if ! command -v jq &> /dev/null; then
    ...
    # Fallback to manual component list for Functorium
    COMPONENTS=(
        "Src/Functorium"
...
generate_filename() {
    local component="$1"
    # Replace / with -, remove src- or Src- prefix (case-insensitive), remove trailing -
    echo "$component" | sed 's|/|-|g' | sed 's|^[Ss]rc-||' | sed 's|-$||'
}
