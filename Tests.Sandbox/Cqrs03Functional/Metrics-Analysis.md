...
이 문서는 `Cqrs03Functional.Demo` 애플리케이션에서 수집되는 Metrics를 분석하여, **Usecase**와 **IObservablePort** 인터페이스 중심의 두 가지 Metrics 범주로 구분하고 각각의 메트릭 이름, 타입, 단위, 태그를 정리합니다.

## 목차

1. [Metrics 범주 개요](#metrics-범주-개요)
2. [Usecase Metrics 형식](#usecase-metrics-형식)
3. [IObservablePort Metrics 형식](#iport-metrics-형식)
...
### 2. IObservablePort Metrics
- **생성 위치**: 소스 생성기로 자동 생성된 `*Pipeline` 클래스에서 `IObservablePortMetric` 호출
- **대상**: `IObservablePort` 인터페이스를 구현한 Repository 등의 Adapter
...
## IObservablePort Metrics 형식

IObservablePort Metrics는 소스 생성기(`ObservablePortGenerator`)에 의해 자동 생성된 Pipeline 클래스에서 `IObservablePortMetric` 인터페이스를 통해 수집됩니다. `[GenerateObservablePort]` 애트리뷰트가 적용된 클래스에 대해 자동으로 Metrics 수집이 활성화됩니다.

**중요**: IObservablePort Metrics는 **RequestCategory별로 Meter와 Metrics가 분리**되어 관리됩니다.
...
| `request.category` | RequestCategory 값 | 요청 카테고리 (IObservablePort.RequestCategory 속성 값) | "Repository", "Db" |
...
#### 수집 시점
- 요청이 들어올 때마다 `_portMetric.Request(activity, requestCategory, requestHandler, requestHandlerMethod, startTime)` 호출
...
#### 수집 시점
- 응답이 성공일 때 `_portMetric.ResponseSuccess(activity, requestCategory, elapsed)` 호출
...
#### 수집 시점
- 응답이 실패일 때 `_portMetric.ResponseFailure(activity, requestCategory, elapsed, error)` 호출
...
| 메트릭 타입 | 공통 태그 | 추가 태그 |
|------------|----------|----------|
| **Request Counter** | `request.layer`, `request.category`, `request.category.type`, `request.handler`, `request.handler.method` | - |
| **Response Success Counter** | `request.layer`, `request.category`, `request.category.type`, `request.handler` | `response.status: Success` |
| **Response Failure Counter** | `request.layer`, `request.category`, `request.category.type`, `request.handler` | `response.status: Failure` |
| **Duration Histogram** | `request.layer`, `request.category`, `request.category.type`, `request.handler`, `request.handler.method` | - |

### IObservablePort Metrics 태그 매핑
...
- **IObservablePort Metrics 구현**: `Src/Functorium/Adapters/Observabilities/Metrics/AdapterMetric.cs`
- **IObservablePort Pipeline 생성기**: `Src/Functorium.SourceGenerators/ObservablePortGenerator.cs`
