...
1. [로그 범주 개요](#로그-범주-개요)
2. [Application 레이어 로그 형식](#application-레이어-로그-형식)
3. [Adapter 레이어 로그 형식](#iadapter-레이어-로그-형식)
4. [로그 필드 매핑](#로그-필드-매핑)
...
아키텍처 기반 프레임워크에서 출력되는 로그는 크게 두 가지 범주로 구분됩니다:

### 1. Application 레이어 로그
...
Adapter 레이어 로그는 소스 생성기(`ObservablePortGenerator`)에 의해 자동 생성된 Pipeline 클래스에서 출력됩니다. `[GenerateObservablePort]` 애트리뷰트가 적용된 클래스에 대해 자동으로 Pipeline이 생성됩니다.

**중요**: Adapter 레이어 로그는 **MinimumLevel 설정에 따라 출력 형식이 달라집니다**:
