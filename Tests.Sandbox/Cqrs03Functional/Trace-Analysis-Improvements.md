...
- IObservablePort: `Adapter Repository InMemoryProductRepository.ExistsByName` (공백 포함)
- Activity 이름에 공백이 포함되어 일부 도구에서 파싱 어려움 가능

**현재 상태**:
```
Usecase:   "Application Usecase.Command CreateProductCommand.Handle"
IObservablePort:    "Adapter Repository InMemoryProductRepository.ExistsByName"
```
...
### 5. Usecase와 IObservablePort의 에러 태그 불일치

**문제점**:
- Usecase: 상세한 에러 태그 (`error.type`, `error.code`, `error.message`, `error.count`)
- IObservablePort: 에러 태그 없음 (StatusDescription만 설정)
...
IObservablePort 에러 태그:
  - 없음 ❌
  - StatusDescription만: error.Message
```

**개선 방안**:
- IObservablePort에도 동일한 에러 태그 구조 적용
- 또는 IObservablePort는 간소화된 에러 정보만 기록하는 것을 명시

**영향도**: 중간 - 에러 분석 시 Usecase/IObservablePort 간 불일치
...
- IObservablePort: `request.category.type` 태그 없음
- 태그 구조가 다름
...
- IObservablePort: `response.status` 태그 없음 (ActivityStatusCode만 사용)
...
**개선 방안**:
- IObservablePort에도 `response.status` 태그 추가
...
**문제점**:
- IObservablePort의 Parent Context 결정이 복잡함 (Traverse Activity 우선, 없으면 Usecase Activity)
...
**문제점**:
- IObservablePort는 `DateTimeOffset.UtcNow` 사용
...
- IObservablePort: Activity 생성 시 `ActivityTagsCollection`으로 한 번에 전달
- 성능 차이 발생 가능
...
3. **태그 구조 통일**: Usecase/IObservablePort 간 공통 태그 구조 통일
4. **에러 태그 통일**: Usecase/IObservablePort 간 에러 태그 구조 통일
