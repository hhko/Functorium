...
Write-Host "=== Cqrs06Services 메시징 테스트 ===" -ForegroundColor Cyan
Write-Host ""

# 1. Docker Compose로 RabbitMQ 시작
Write-Host "[1/3] RabbitMQ 컨테이너 시작 중..." -ForegroundColor Yellow
$composeFile = "Tutorials/Cqrs06Services/docker-compose.yml"
...
$inventoryJob = Start-Job -ScriptBlock {
    Set-Location "E:\2025\Dev\DDD-Course\2025-12-30_Functorium\Tutorials\Cqrs06Services\Src\InventoryService"
    dotnet run 2>&1
}
...
Set-Location "Tutorials/Cqrs06Services/Src/OrderService"
dotnet run 2>&1
