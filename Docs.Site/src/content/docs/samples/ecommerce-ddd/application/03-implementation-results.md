...
### 시나리오 4: PlaceOrder -- 다중 Aggregate 쓰기 (UoW)
...
### 시나리오 5: SearchProducts -- Specification 합성 + 페이지네이션
...
### 시나리오 7: AlreadyExists (중복 이름/이메일)
...
### 시나리오 9: 도메인 에러 전파 (CreditLimitExceeded, InsufficientStock)
...
| 테스트 클래스 | 검증 대상 | 핵심 규칙 |
|-------------|----------|----------|
| `UsecaseArchitectureRuleTests` | 11 Commands + 9 Queries | 중첩 `Usecase` 클래스 (sealed, `ICommandUsecase`/`IQueryUsecase`), 선택적 `Validator` 클래스 (sealed, `AbstractValidator`) |
