...
[비즈니스 요구사항](./00-business-requirements/)에서 정의한 10개 시나리오를, [코드 설계](./02-code-design/)에서 매핑한 타입 구조로 실행합니다. 정상 시나리오(1~6)는 타입이 올바른 상태를 어떻게 표현하는지, 거부 시나리오(7~10)는 잘못된 상태를 어떻게 차단하는지 확인합니다.
...
`ContactEmailCheckService`는 최소 데이터(`Seq<(ContactId, string?)>`)만 받아 교차 Aggregate 검증을 수행합니다. 다른 이메일(`other@example.com`)은 통과하고, 이미 존재하는 이메일(`user@example.com`)은 거부됩니다. 자기 자신의 ID를 전달하면 업데이트 시나리오에서 자기 제외가 적용됩니다.
...
```
ContactEmailSpec.IsSatisfiedBy: True
다른 이메일 Spec: False
ContactEmailUniqueSpec (전체): True
ContactEmailUniqueSpec (자기 제외): False
```

- **`ContactEmailSpec`은** `EmailValue` 투영 속성으로 이메일 일치 여부를 판별합니다. 일치하면 `True`, 불일치하면 `False`입니다.
- **`ContactEmailUniqueSpec`은** 이메일 일치 + ID 제외 조건을 결합합니다. 자기 자신의 ID를 제외하면 해당 Contact는 `False`가 되어 중복으로 판정되지 않습니다.

두 Specification 모두 `Expression<Func<Contact, bool>>`을 반환하므로, ORM에서 SQL로 변환하여 데이터베이스 수준에서 필터링할 수 있습니다.
...
| 10. 중복 이메일 등록 | 이메일 고유성 | `ContactEmailCheckService` + 자기 제외 | `EmailAlreadyInUse` |

## Naive에서 타입 안전 도메인 모델로

Naive `Contact` 클래스의 모든 `string` 필드는 제약된 타입으로 대체되었고, `bool IsEmailVerified`는 union 기반 상태 머신으로 진화했습니다. 10개 시나리오 중 하나(시나리오 7)는 컴파일 타임에, 나머지 거부 시나리오(8~10)는 구조적으로 정의된 에러 타입과 `Fin<T>` 반환으로 런타임에 차단됩니다. 114개 단위 테스트(`DesigningWithTypes.Tests.Unit/`)가 이 보장을 지속적으로 검증합니다.
...
| Domain Service | 교차 Aggregate 검증 | `ContactEmailCheckService`가 이메일 고유성을 Aggregate 외부에서 검증 |
