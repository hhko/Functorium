...
`InMemoryRepositoryBase<TAggregate, TId>`는 `ConcurrentDictionary` 기반으로 `IRepository` 전체 8개 CRUD를 기본 구현하는 프레임워크 베이스 클래스입니다.
서브클래스는 `Store` 프로퍼티만 제공하면 되며, Soft Delete 등 특수 로직이 필요한 메서드만 오버라이드합니다.

The key point to note in the following code is 베이스 클래스가 CRUD를 기본 제공하고, Soft Delete가 필요한 `GetById`와 `Delete`만 오버라이드하는 구조입니다.
...
| 멤버 | Type | Description |
|------|------|------|
| `Store` | `abstract ConcurrentDictionary<TId, T>` | 서브클래스가 제공하는 인메모리 저장소 |
| `Create` / `CreateRange` | `virtual` | 저장 + 이벤트 추적 |
| `GetById` / `GetByIds` | `virtual` | 조회 + Not Found 에러 |
| `Update` / `UpdateRange` | `virtual` | 갱신 + 이벤트 추적 |
| `Delete` / `DeleteRange` | `virtual` | 삭제 (반환: `int` — 영향 행 수) |
| `NotFoundError()` | `protected` | 에러 헬퍼 |
| `EventCollector` | `protected` | 도메인 이벤트 수집기 |

**Repository Adapter 핵심 패턴**:

| Pattern | 코드 | Description |
|------|------|------|
...
| 삭제 반환 | `Fin.Succ(1)` / `Fin.Succ(0)` | 영향 행 수 (`int`) |
...
> **Reference**: 파이프라인 패턴은 [11-usecases-and-cqrs.md §트랜잭션과 이벤트 발행](../application/11-usecases-and-cqrs#트랜잭션과-이벤트-발행-usecasetransactionpipeline)을 참조하세요.
...
The key point to note in the following code is HTTP 상태 코드별 에러 매핑(`HandleHttpError`)과 예외 유형별 `AdapterError` 변환 구조입니다.
...
| HTTP 상태 코드 | AdapterErrorType | Description |
|---------------|------------------|------|
...
| 예외 타입 | AdapterErrorType | Description |
|----------|------------------|------|
...
`InMemoryQueryBase<TEntity, TDto>`는 `DapperQueryBase`의 InMemory 대응 베이스 클래스입니다.
서브클래스는 **데이터 소스 접근(`GetProjectedItems`)과 정렬 키(`SortSelector`)만** 담당하고, Search/Stream/페이지네이션은 베이스가 처리합니다.
...
**Cause:** Pipeline 클래스가 원본 Adapter 클래스를 상속받아 메서드를 `override`합니다. `virtual`이 없으면 override가 불가능하여 `CS0506: cannot override because it is not virtual` 에러가 발생합니다.
...
**Cause:** `IO.lift`는 동기 람다만 허용합니다. 내부에서 `await`를 사용하려면 `IO.liftAsync`를 사용해야 합니다.
...
**Cause:** Adapter 내부의 Mapper 클래스가 `public`으로 선언되면 외부 프로젝트에서 기술 관심사 변환 로직에 접근할 수 있어 레이어 경계가 무너집니다.
...
| 문서 | Description |
|------|------|
| [12-ports.md](./12-ports) | Port 아키텍처, IObservablePort 계층, Port 정의 규칙 |
| [14a-adapter-pipeline-di.md](./14a-adapter-pipeline-di) | Pipeline 생성, DI 등록, Options 패턴 |
| [14b-adapter-testing.md](./14b-adapter-testing) | Adapter 단위 테스트, E2E Walkthrough |
...
- Port 인터페이스 정의 규칙: [Port 정의](./12-ports)
- Pipeline 생성과 DI 등록: [Adapter 연결 — Pipeline과 DI](./14a-adapter-pipeline-di)
- Adapter 단위 테스트: [Adapter 테스트](./14b-adapter-testing)
