...
"Application Layer가 데이터베이스나 외부 API에 직접 의존하면 어떤 문제가 생기는가?"
"Repository 인터페이스는 Domain Layer에, External API 인터페이스는 Application Layer에 두는 이유는 무엇인가?"
"Port 메서드의 반환 타입을 `FinT<IO, T>`로 통일하면 어떤 이점이 있는가?"
...
다음 코드에서 주목할 점은 CRUD 메서드를 재선언하지 않고, 도메인에 특화된 `Exists`와 `GetByIdIncludingDeleted`만 추가하는 것입니다.
