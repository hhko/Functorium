...
The key point to note in the following code is Usecase가 Repository만 주입받고, SaveChanges와 이벤트 발행을 직접 호출하지 않는다는 것입니다.
...
> **Reference**: 파이프라인 상세는 [11-usecases-and-cqrs.md §트랜잭션과 이벤트 발행](../application/11-usecases-and-cqrs#트랜잭션과-이벤트-발행-usecasetransactionpipeline)을 참조하세요.
...
**Cause:** `UsecaseTransactionPipeline`은 SaveChanges 성공 후 이벤트를 발행합니다. 핸들러에서 예외가 발생하면 이벤트 발행은 실패하지만 데이터는 이미 커밋된 상태입니다 (eventual consistency).
