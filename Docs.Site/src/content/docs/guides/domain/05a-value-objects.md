...
This document covers the design and implementation of Value Objects that express domain concepts as types, going beyond the limitations of primitive types. 열거형 패턴, Application 검증, FAQ는 [05b-value-objects-validation](./05b-value-objects-validation), Union 타입(Discriminated Union)은 [05c-union-value-objects](./05c-union-value-objects)를 참고하세요.
...
1. **원시 타입 대신 값 객체를 사용해야 하는 이유** - Primitive Obsession 문제와 해결 방법
2. **기반 클래스 Optional 기준** - `SimpleValueObject<T>`, `ValueObject`, `ComparableSimpleValueObject<T>` 등의 사용 시나리오
3. **Create/Validate 분리 패턴** - 검증 로직 재사용을 위한 핵심 설계
4. **검증 시스템 전체 구조** - 순차 검증(Bind), 병렬 검증(Apply), 세 가지 검증 방식
5. **FluentValidation 통합** - Domain Layer 검증 로직을 Application Layer에서 재사용
...
- C# 제네릭, 정적 팩토리 메서드 패턴
- LanguageExt의 `Fin<T>`, `Validation<Error, T>` 기본 개념

> In DDD tactical design, Value Objects are **the most fundamental building block for explicitly expressing domain concepts.** 원시 타입 대신 값 객체를 사용하면, 컴파일 타임에 타입 안전성을 확보하고, 검증 로직을 한 곳에 캡슐화하여 중복을 제거할 수 있습니다.
...
1. 기반 클래스 Optional (`SimpleValueObject<T>`, `ComparableSimpleValueObject<T>`, `ValueObject`)
...
DDD 전술적 설계에서 값 객체는 **도메인 개념을 명시적으로 표현하는 가장 기본적인 빌딩블록입니다.**
...
The following table 원시 타입과 값 객체의 대응 관계를 보여줍니다. The key difference is 값 객체가 컴파일 타임에 타입 안전성을 보장한다는 점입니다.
...
Value Objects validate at creation time, ensuring that invalid values cannot exist in the system. 한 번 생성된 값 객체는 항상 유효합니다.

### Eliminating Side Effects Through Immutability

Value Objects cannot be changed after creation, making them thread-safe and predictable. 값을 변경해야 할 때는 새로운 값 객체를 생성합니다.

### Effects of Value Objects in Practice

값 객체를 도입하기 전과 후를 비교하면, 코드의 안전성과 의도 전달에서 명확한 차이가 드러납니다.

**도입 전**: `ProcessOrder(string email, decimal price, string currency)` 시그니처에서는 파라미터 순서를 착각해도 컴파일러가 잡지 못합니다. 가격에 음수가 들어와도 호출 시점에서는 알 수 없고, 검증 로직이 여러 계층에 분산됩니다.

**도입 후**: `ProcessOrder(Email email, Money price)` 시그니처에서는 타입 자체가 유효성을 보장합니다. `Email.Create("invalid")`는 실패를 반환하고, `Money.Create(-100)`은 양수만 허용하므로 잘못된 값이 시스템에 진입할 수 없습니다.
...
지금까지 값 객체가 필요한 이유와 판단 기준을 살펴보았습니다. 다음 섹션에서는 값 객체의 핵심 특징과 기반 클래스 Optional 방법을 알아봅니다.

---

## Overview

값 객체(Value Object)는 도메인 주도 설계(DDD)의 핵심 전술 패턴 중 하나입니다. "이메일 주소", "가격", "금액" 같은 도메인 개념을 **원시 타입(string, decimal) 대신 전용 타입으로 표현합니다.**
...
| **Domain logic encapsulation** | 관련 연산(비교, 변환, 계산)을 타입 내부에 포함 |

### Base Classes Optional

| 사용 시나리오 | 기반 클래스 | Features |
|--------------|------------|------|
...
| Composite properties + 비교 | `ComparableValueObject` | Supports sorting and comparison operations |
| 단일 값 + 비교 | `ComparableSimpleValueObject<T>` | Supports sorting and comparison operations |
| 타입 안전한 열거형 | `SmartEnum<T, TValue>` (Ardalis.SmartEnum) | Enumeration with built-in domain logic. IValueObject 수동 구현 필요 |

### Core Pattern

The key point in the following code is `Create()`와 `Validate()`가 분리되어 있다는 것입니다. `Validate()`는 원시 타입을 반환하므로 FluentValidation에서 재사용할 수 있고, `Create()`는 `Validate()` 결과를 받아 객체를 생성합니다.
...
개요에서 값 객체의 핵심 특징과 기반 클래스 Optional 가이드를 확인했습니다. 다음 섹션에서는 Functorium이 제공하는 클래스 계층 구조를 상세히 살펴봅니다.
...
- **ValueObject**: Composite properties 값 객체의 기반. `CreateFromValidation` 헬퍼 메서드를 제공합니다.
- **SimpleValueObject\<T\>**: Single value wrapping용. `GetEqualityComponents()`가 자동 구현됩니다.
...
The first thing to do when implementing a Value Object is to decide **which base class to inherit from.** 아래 두 가지 질문으로 쉽게 Optional할 수 있습니다:
...
Composite properties으로 구성된 값 객체의 기반 클래스입니다. 여러 속성의 조합이 하나의 개념을 표현할 때 사용합니다 (예: 금액 + 통화 = Money).
...
The key point in the following code is `GetEqualityComponents()`에서 `Amount`와 `Currency`를 모두 반환하여, 두 속성의 조합으로 동등성을 판단한다는 것입니다.
...
단일 값을 래핑하는 값 객체의 기반 클래스입니다. **가장 많이 사용되는 기반 클래스로,** 하나의 원시 타입에 도메인 의미를 부여할 때 사용합니다. `GetEqualityComponents()`가 자동 구현되어 `Value`를 반환합니다.
...
비교 가능한 복합 값 객체의 기반 클래스입니다. Composite properties이면서 정렬이나 크기 비교가 필요할 때 사용합니다 (예: 시작일+종료일로 구성된 DateRange의 기간 비교).
...
기반 클래스 Optional과 구현 방법을 확인했습니다. 다음 섹션에서는 값 객체의 핵심인 Self-validation 시스템을 상세히 다룹니다.

---

## Validation System

값 객체의 **Self-validation(Self-Validation)** 원칙을 구현하는 시스템입니다. 모든 값 객체는 생성 시점에 유효성을 검증하여 **잘못된 상태의 객체가 존재할 수 없도록** 보장합니다.
...
> **Context Class는** `IValidationContext`를 구현한 빈 클래스입니다. Application Layer에서 검증 컨텍스트를 재사용할 때 사용합니다. 자세한 내용은 [IValidationContext를 이용한 검증](#ivalidationcontext를-이용한-검증-context-class) 섹션을 참조하세요.
...
- Application Layer에서 **재사용 가능한 검증 컨텍스트가** 필요할 때
- Named Context의 **문자열 오타 위험을** 피하고 싶을 때
...
The following table 세 가지 검증 방식을 비교합니다. The key difference is 타입 안전성 수준과 권장 사용 레이어입니다.
...
Application Layer에서 Request DTO를 검증할 때 FluentValidation을 많이 사용합니다. 이때 **값 객체에 이미 정의된 검증 로직을 재사용하면** 중복을 피하고 일관성을 유지할 수 있습니다.

Functorium은 FluentValidation과의 통합을 위해 `MustSatisfyValidation` 확장 메서드를 제공합니다. 이를 통해 **검증 로직을 Domain Layer(Value Object)에서 한 번만 정의하고,** Application Layer에서 그대로 재사용할 수 있습니다.
...
검증 로직을 각각 따로 작성하면 **중복이** 발생하고, 규칙이 변경될 때 **두 곳을 모두 수정해야** 합니다.
...
대부분의 경우 **MustSatisfyValidation을** 사용합니다. Value Object의 `Validate` 메서드는 보통 같은 타입을 입력받고 반환하기 때문입니다.
...
Functorium은 **예외 대신 결과 타입을** 사용하여 오류를 처리합니다. 검증 실패는 예외를 던지지 않고 `Validation<Error, T>` 또는 `Fin<T>`로 표현됩니다. 이 방식은 함수형 프로그래밍의 "실패는 예외가 아닌 정상적인 결과"라는 철학을 따릅니다.

**결과 타입의 장점:**
- 호출자가 실패 가능성을 **명시적으로** 처리해야 함 (컴파일러가 강제)
- try-catch 없이 **함수 체이닝으로** 오류 처리 가능
- 여러 오류를 **수집하여** 한 번에 반환 가능

> **상세 내용은 [에러 시스템 가이드](./08a-error-system)를 참조하세요.**
...
| Category | Description | 대표 ErrorType |
|------|------|---------------|
...
값 객체 구현의 핵심은 **Create/Validate 분리 패턴입니다.** 이 패턴은 검증 로직과 객체 생성을 분리하여 재사용성과 테스트 용이성을 높입니다.
...
**Cause:** `Validation<Error, T>` 튜플에 Apply를 사용할 때 LanguageExt의 타입 추론이 실패하는 경우가 있습니다.
**Solution:** 튜플에 `TypedValidation`이 포함되어 있으면 `.As()` 없이 사용 가능합니다. 그 외의 경우 `ValidationApplyExtensions` 네임스페이스(`Functorium.Domains.ValueObjects.Validations`)를 추가하세요. 이 네임스페이스의 Apply 오버로드는 `.As()` 없이 직접 동작합니다.

### MustSatisfyValidation에서 타입 추론 실패
**Cause:** `MustSatisfyValidationOf`는 입력 타입과 출력 타입이 다를 때 사용하는데, C#14 extension members의 타입 추론 제한으로 제네릭 파라미터를 명시해야 합니다.
**Solution:** 입력/출력 타입이 동일하면 `MustSatisfyValidation`을 사용하세요. 다를 경우 `MustSatisfyValidationOf<TRequest, TIn, TOut>(Validate)` 형태로 타입을 명시하세요.

### ValidationRules 네임스페이스를 찾을 수 없음
**Cause:** 검증 방식(Typed, Contextual, Context Class)에 따라 네임스페이스가 다릅니다.
**Solution:**
