...
> **Core principle:** 메트릭은 로그와 달리 집계된 숫자를 저장하여 "얼마나 많이/빠르게?"라는 질문에 효율적으로 답합니다. Functorium은 Counter(요청/응답 수)와 Histogram(processing time 분포)을 자동 수집하며, `response.elapsed`는 카디널리티 폭발을 방지하기 위해 태그가 아닌 Histogram으로 기록합니다.
...
10:30:02 주문 #1002 처리 failure (재고 부족)
...
로그로 "지난 1시간 동안 주문 processing time이 증가했는가?"라는 질문에 답하려면 수천 개의 로그를 읽고 파싱해야 합니다. 메트릭은 이미 집계된 데이터를 저장하므로 단순한 쿼리로 즉시 답할 수 있습니다.
...
- `responses` Counter: 총 응답 수 (success/failure 구분)
...
> tag key도 동일하게 변환됩니다 (`response.status` → `response_status`).
...
**Functorium에서:**
- `duration` Histogram: processing time 분포
...
**Note:** Functorium의 자동 메트릭에는 Gauge가 포함되지 않습니다. 비즈니스 요구사항에 따라 사용자 정의 Gauge를 추가할 수 있습니다.
...
만약 processing time(예: 0.0234초)을 태그로 사용하면:
- 0.0234초 → 시계열 1
- 0.0235초 → 시계열 2
- 0.0236초 → 시계열 3
- ...무한히 증가
...
processing time은 `duration` Histogram으로 기록됩니다. Histogram은 값을 **버킷(bucket)으로** 그룹화합니다:
...
Meter는 관련된 Instrument들의 논리적 그룹입니다. OpenTelemetry에서 Meter는 일반적으로 라이브러리나 컴포넌트 unit로 생성됩니다.
...
응답 완료 시점에 증가합니다. success/failure가 `response.status` 태그로 구분됩니다.
...
processing time의 분포를 기록합니다. 초(second) unit로 측정됩니다.
...
태그(Tag)는 메트릭을 다양한 차원으로 분석할 수 있게 해주는 메타데이터입니다. Functorium은 로깅과 동일한 tag key를 사용하여 일관성을 유지합니다.
...
3. **processing time 태그 제외**: `response.elapsed`는 연속적인 값이므로 태그가 아닌 Histogram으로 기록합니다.
...
- error.code: M (error code 수)

최대 카디널리티 ≈ 1 × 1 × 2 × N × 1 × 2 × 3 × M = 12 × N × M

핸들러 100개, error code 50개 가정:
...
| Type | Counter |
| unit | {request} |
| Description | Usecase 요청 수 |
| recorded at | 핸들러 실행 시작 시 |
...
| Type | Counter |
| unit | {response} |
| Description | Usecase 응답 수 (success/failure 구분) |
| recorded at | 핸들러 실행 완료 시 |
...
| Type | Histogram |
| unit | s (seconds) |
| Description | Usecase processing time 분포 |
| recorded at | 핸들러 실행 완료 시 |
...
- `RequestDuration`: 요청 processing time을 측정하기 위한 헬퍼 클래스. `using` 구문과 함께 사용하여 Histogram에 자동 기록합니다.
...
| Type | Counter |
| unit | {request} |
| Description | Adapter 요청 수 |
| recorded at | 메서드 실행 시작 시 |
...
| Type | Counter |
| unit | {response} |
| Description | Adapter 응답 수 (success/failure 구분) |
| recorded at | 메서드 실행 완료 시 |
...
| Type | Histogram |
| unit | s (seconds) |
| Description | Adapter processing time 분포 |
| recorded at | 메서드 실행 완료 시 |
...
| "aggregate" | 여러 오류 결합 | 첫 번째 error code로 분류 |
...
1. 불필요한 태그 제거
2. 태그 값 정규화 (예: 에러 메시지 대신 error code 사용)
3. 특정 조건에서만 메트릭 수집
