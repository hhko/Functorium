...
> **Core principle:** 메트릭은 로그와 달리 집계된 숫자를 저장하여 "얼마나 많이/빠르게?"라는 질문에 효율적으로 답합니다. Functorium은 Counter(요청/응답 수)와 Histogram(처리 시간 분포)을 자동 수집하며, `response.elapsed`는 카디널리티 폭발을 방지하기 위해 태그가 아닌 Histogram으로 기록합니다.
...
| Type | Counter |
| 단위 | {response} |
| Description | Usecase 응답 수 (성공/실패 구분) |
| 기록 시점 | 핸들러 실행 완료 시 |
...
| Type | Counter |
| 단위 | {response} |
| Description | Adapter 응답 수 (성공/실패 구분) |
| 기록 시점 | 메서드 실행 완료 시 |
...
| Type | Counter |
| 단위 | {response} |
| Description | DomainEvent 발행 응답 수 (성공/실패 구분) |
...
| Type | Histogram |
| 단위 | s (seconds) |
| Description | DomainEvent 발행 처리 시간 분포 |
| 기록 시점 | Publisher 실행 완료 시 |
...
| Type | Counter |
| 단위 | {response} |
| Description | DomainEvent Handler 응답 수 (성공/실패 구분) |
...
| Type | Histogram |
| 단위 | s (seconds) |
| Description | DomainEvent Handler 처리 시간 분포 |
| 기록 시점 | Handler 실행 완료 시 |
