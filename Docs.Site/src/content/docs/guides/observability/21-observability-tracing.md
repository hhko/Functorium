...
> **핵심 원칙:** 분산 추적은 하나의 요청이 시스템을 통과하는 전체 여정을 Parent-Child Span 관계로 시각화합니다. Functorium은 Application Layer와 Adapter Layer에서 자동으로 Span을 생성하며, Expected 에러도 Error 상태로 기록하되 `error.type` 태그로 성격을 구분합니다.
