...
> **Core principle:** 분산 추적은 하나의 요청이 시스템을 통과하는 전체 여정을 Parent-Child Span 관계로 시각화합니다. Functorium은 Application Layer와 Adapter Layer에서 자동으로 Span을 생성하며, Expected 에러도 Error 상태로 기록하되 `error.type` 태그로 성격을 구분합니다.
...
| Span | Trace 내의 개별 작업 unit (시작 시간, 소요 시간, 태그 포함) |
| Parent-Child | Usecase Span이 Adapter Span의 부모 - 계층적 호출 구조 표현 |
| Span Name | Application: `{layer} {category}.{cqrs} {handler}.{method}`, Adapter: `{layer} {category} {handler}.{method}` |
| Status | `Ok`/`Error` - Expected 에러도 Error 상태 (`error.type` 태그로 구분) |
| `response.elapsed` | 트레이싱에서는 태그로 포함 (개별 Span 문서 저장이므로 카디널리티 문제 없음) |
...
Span은 Trace 내의 **개별 작업 unit**입니다. 하나의 Trace는 여러 개의 Span으로 구성됩니다. 각 Span은 "언제 시작했고, 얼마나 걸렸는가"를 기록합니다.
...
| **상태** | success/failure 여부 | Ok / Error |
...
Span의 Status는 작업의 success/failure를 나타냅니다:

| Status | Description | 언제 사용 |
|--------|------|-----------|
| **Unset** | 상태 미설정 | Default |
| **Ok** | success | 정상 처리 완료 |
| **Error** | failure | 에러 발생 |
...
태그는 Span에 추가적인 컨텍스트를 제공합니다. Functorium은 로깅, 메트릭과 동일한 tag key를 사용하여 **3-Pillar 일관성**을 유지합니다.
...
| `error.code` | - | error code | 도메인 error code |
| **total tag count** | **7** | **9** | |
...
| **저장 방식** | 태그 조합별 시계열 | Span 문서 unit |

트레이싱에서 각 Span은 개별 문서로 저장됩니다. `response.elapsed` 값이 다르다고 별도 시계열이 생성되지 않습니다. 따라서 태그로 포함해도 카디널리티 문제가 없습니다.

또한, 개별 Span에서 정확한 processing time을 태그로 확인하면 특정 요청의 성능을 빠르게 파악할 수 있습니다.

### ctx.* Span Attribute — 사용자 정의 비즈니스 컨텍스트

`CtxEnricherPipeline`이 최선두에서 실행되어, `CtxPillar.Tracing` 플래그가 포함된 ctx.* 필드를 `Activity.Current?.SetTag`으로 Span Attribute에 자동 설정합니다. **기본값(`CtxPillar.Default = Logging | Tracing`)으로 모든 ctx.* 필드는 Span Attribute에 포함됩니다.**
...
Span은 개별 저장되므로 고카디널리티 필드(`customer_id`, `Guid`)도 Span Attribute로 안전합니다. OpenTelemetry는 디버깅을 위해 Span에 풍부한 attribute를 권장합니다.

**Tracing에서 제외하려면:**

```csharp
[CtxTarget(CtxPillar.Logging)]  // Logging 전용 — Tracing Span에 포함하지 않음
string InternalNote
```
...
- `SetStandardRequestTags(activity, method)`: 5개 표준 요청 태그를 자동 설정합니다:
  ...
  - `request.handler.name` (Handler 이름)
...
Source Generator는 `[ObservabilityPipeline]` attribute이 적용된 인터페이스에 대해 자동으로 트레이싱 코드를 생성합니다.
...
| `response.elapsed` | - | processing time(초) | processing time(초) |
| `response.status` | - | "success" | "failure" |
| `error.type` | - | - | "expected"/"exceptional" |
| `error.code` | - | - | error code |
| **total tag count** | **4** | **6** | **8** |

### Publisher 태그 구조 (PublishTrackedEvents)

추적된 Aggregate 이벤트 발행 시의 태그 구조:
...
| `response.elapsed` | - | processing time(초) | processing time(초) | processing time(초) |
...
- **`request.event.type`**: 이벤트 타입명. `request.handler.name`(handler name)와 **다른 값**입니다.
...
**`request.event.type` vs `request.handler.name` 관계:**

`request.handler.name`는 이벤트를 처리하는 **핸들러 클래스**를, `request.event.type`은 핸들러가 구독하는 **이벤트 타입**을 나타냅니다. 하나의 이벤트에 여러 핸들러가 있을 때 이 구분이 중요합니다:

```
# ProductCreatedEvent를 두 개의 핸들러가 구독하는 경우:

Span 1: application usecase.event OnProductCreated.Handle
  request.handler.name = "OnProductCreated"       ← 핸들러 클래스
  request.event.type   = "ProductCreatedEvent"    ← 이벤트 타입
  request.event.id     = "a1b2c3d4-..."           ← 동일 이벤트 인스턴스

Span 2: application usecase.event SyncInventoryOnProductCreated.Handle
  request.handler.name = "SyncInventoryOnProductCreated"  ← 다른 핸들러
  request.event.type   = "ProductCreatedEvent"            ← 동일 이벤트 타입
  request.event.id     = "a1b2c3d4-..."                   ← 동일 이벤트 인스턴스
```

`request.event.id`가 같으므로 두 Span이 동일 이벤트 인스턴스에 의해 트리거되었음을 알 수 있습니다.
...
> **Note:** Handler의 `error.code`는 예외 타입명(`InvalidOperationException`), Publisher의 `error.code`는 래핑된 error code(`ApplicationErrors.DomainEventPublisher.PublishFailed`)가 기록됩니다.
...
| attribute | 의미 | Value |
|------|------|-----|
| **Status** | 작업 success 여부 | Ok / Error |
| **error.type** | 에러의 성격 | expected / exceptional / aggregate |

**예시:**

| 시나리오 | Status | error.type | Description |
|----------|--------|------------|------|
| 주문 success | Ok | - | 정상 처리 |
| 재고 부족 | Error | expected | 비즈니스 규칙에 따른 거부 |
| DB 연결 failure | Error | exceptional | 시스템 문제 |
...
자식 Span에서 에러가 발생하면, 부모 Span도 일반적으로 Error 상태가 됩니다. 이는 "자식의 failure가 부모의 failure를 유발"하기 때문입니다.
