...
**`request.event.type` vs `request.handler` 관계:**

`request.handler`는 이벤트를 처리하는 **핸들러 클래스**를, `request.event.type`은 핸들러가 구독하는 **이벤트 타입**을 나타냅니다. 하나의 이벤트에 여러 핸들러가 있을 때 이 구분이 중요합니다:

```
# ProductCreatedEvent를 두 개의 핸들러가 구독하는 경우:

Span 1: application usecase.event OnProductCreated.Handle
  request.handler     = "OnProductCreated"       ← 핸들러 클래스
...
Span 2: application usecase.event SyncInventoryOnProductCreated.Handle
  request.handler     = "SyncInventoryOnProductCreated"  ← 다른 핸들러
