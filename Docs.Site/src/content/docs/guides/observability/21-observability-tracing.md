...
| Span | Trace 내의 개별 작업 단위 (시작 시간, 소요 시간, 태그 포함) |
...
| 태그 키 | 성공 | 실패 | Description |
|---------|------|------|------|
| `request.layer` | "application" | "application" | 레이어 식별자 |
| `request.category.name` | "usecase" | "usecase" | 카테고리 식별자 |
| `request.category.type` | "command"/"query" | "command"/"query" | CQRS 타입 |
| `request.handler.name` | 핸들러명 | 핸들러명 | 핸들러 클래스명 |
...
| `request.category.name` | 카테고리명 | 카테고리명 | 카테고리 식별자 |
| `request.handler.name` | 핸들러명 | 핸들러명 | 핸들러 클래스명 |
...
- **`request.event.type`**: 이벤트 타입명. `request.handler.name`(핸들러명)와 **다른 값**입니다.
