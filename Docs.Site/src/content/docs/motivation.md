...
위의 세 가지 문제는 각각 독립적이지 않습니다. 도메인 로직이 예외로 표현되면 로그에 일관된 도메인 언어를 반영하기 어렵고, Observability가 사후에 추가되면 도메인 흐름과 관측 정보가 따로 놀게 됩니다. Functorium은 이 세 문제를 하나의 아키텍처로 통합하여 해결합니다.
...
`FinT<IO, T>`는 여기에 사이드 이펙트 추적을 더합니다. IO 작업(데이터베이스 조회, 외부 API 호출)이 타입에 명시되므로, 순수 함수와 부수 효과를 가진 함수가 시그니처 수준에서 구분됩니다. 이를 통해 도메인 로직은 순수하게 유지되고, 사이드 이펙트는 파이프라인의 경계에서 관리됩니다.
...
이 세 가지 문제를 해결하기 위한 구체적인 설계 원칙은
