...
API를 문서화할 때는 반드시 다음 순서를 따릅니다. 먼저 Uber 파일에서 API를 검색하고, 개별 API 파일에서 상세 내용을 확인한 뒤, 완전한 API 시그니처를 추출하고, 올바른 시그니처로 코드 샘플을 작성합니다. 이 순서를 지키지 않으면 존재하지 않는 API를 문서화하는 실수가 발생합니다.
...
실제로 존재하지 않는 메서드나 플루언트 체인을 만들어내는 것은 가장 흔하고 위험한 실수입니다. Uber 파일에 없는 API는 코드 샘플에 절대 사용해서는 안 됩니다.
