...
**A**: 릴리스 노트의 코드 예제에 등장하는 **클래스명, 메서드명, 파라미터 타입이 Uber 파일의 API 시그니처와 정확히 일치하는지** 확인합니다. 예를 들어 코드 예제에서 `ErrorCodeFactory.Create()`를 사용했다면, Uber 파일에 해당 메서드가 동일한 시그니처로 존재해야 합니다.
