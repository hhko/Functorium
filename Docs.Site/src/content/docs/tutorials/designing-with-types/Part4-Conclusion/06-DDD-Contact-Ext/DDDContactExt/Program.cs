...
Console.WriteLine("=== 05-DDD-Contact-Ext: DDD 고급 패턴 적용 ===\n");
...
Console.WriteLine("\n--- 이메일 인증 ---");
contact.VerifyEmail(DateTime.UtcNow).ThrowIfFail();
...
contact.UpdateName(newName).ThrowIfFail();
...
contact.AddNote(noteContent).ThrowIfFail();
...
Console.WriteLine("\n--- 삭제된 Contact에 행위 시도 ---");
var updateResult = contact.UpdateName(name);
Console.WriteLine($"UpdateName: IsFail={updateResult.IsFail}");

var addNoteResult = contact.AddNote(noteContent);
...
Console.WriteLine("\n--- CreateFromValidated ---");
var note = ContactNote.Create(NoteContent.Create("복원 메모").ThrowIfFail());
