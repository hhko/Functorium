...
내부 구현용으로 만든 캐시 클래스가 실수로 `public`이 되면 어떤 일이 벌어질까요? 외부 모듈에서 직접 참조하기 시작하고, 나중에 내부 구현을 바꾸려 할 때 이미 여러 곳에서 의존하고 있어 변경이 불가능해집니다. 이 장에서는 이런 문제를 방지하기 위해 **가시성과 한정자를** 아키텍처 테스트로 강제하는 방법을 학습합니다.

> **"클래스의 가시성과 한정자는 설계 의도의 표현입니다. 아키텍처 테스트는 이 의도가 코드베이스 전체에서 일관되게 유지되도록 보장합니다."**
...
다음 표는 ClassValidator의 가시성 및 한정자 검증 메서드를 정리합니다.
