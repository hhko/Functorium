...
`Email.Create`가 `Fin<Email>`을 반환해야 한다는 규칙, 누군가 그냥 `Email`을 반환하면 어떻게 될까요? 컴파일은 문제없이 통과합니다. 호출하는 쪽에서 `IsSucc`/`IsFail` 분기를 기대하지만, 실제로는 예외가 던져지는 코드가 되어 런타임에 비로소 장애가 발생합니다. 이 장에서는 메서드의 반환 타입을 아키텍처 테스트로 검증하여, **`Fin<T>`와** 같은 함수형 결과 타입의 사용을 강제하거나, 팩토리 메서드가 자신의 클래스 타입을 반환하는지 확인하는 방법을 학습합니다.
...
2. **자기 타입 반환 검증**
   - `RequireReturnTypeOfDeclaringClass()`로 팩토리 메서드가 선언 클래스 타입을 반환하는지 확인
...
다음 표는 반환 타입 검증 API의 매칭 방식과 사용 시나리오를 요약합니다.
...
**A**: 아닙니다. 정확히 선언 클래스와 동일한 타입인지만 검증합니다. `Customer` 클래스의 메서드가 `Customer`를 반환하면 통과하지만, `Customer`의 하위 타입을 반환하면 실패합니다. 이는 팩토리 메서드가 정확한 자기 타입을 반환하도록 강제하는 데 적합합니다.
...
[이전: Chapter 5 - 메서드 검증](../01-Method-Validation/) | [다음: Chapter 7 - 파라미터 검증](../03-Parameter-Validation/)
