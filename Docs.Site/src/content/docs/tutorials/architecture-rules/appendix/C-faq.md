...
아키텍처 테스트를 도입하면서 자주 등장하는 질문과 답변을 모았습니다. ClassValidator, MethodValidator, 커스텀 규칙, 트러블슈팅까지 카테고리별로 정리했으니 막히는 부분이 있을 때 찾아보세요.
...
`typeof(Fin<>)`는 open generic 타입입니다. MethodValidator는 반환 타입의 generic 정의를 추출하여 compares. 예를 들어, `Fin<Email>`의 generic 정의는 `Fin<>`이므로 매칭됩니다.
...
`RequireExtensionMethod()`는 `[ExtensionAttribute]`가 있는지 verifies.
...
C#의 positional record는 기본적으로 `init` setter가 있는 속성을 생성합니다. `init` setter도 setter로 간주되어 immutability 검증에 영향을 줄 수 있습니다. 도메인 record에는 private 생성자와 factory method를 사용하세요.
