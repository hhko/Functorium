...
Command에 `Request`가 빠져 있다면? Query에 `Response`가 없다면? Mediator 파이프라인이 런타임에 실패하고 나서야 "중첩 클래스를 깜빡했다"는 사실을 알게 됩니다. 이런 구조적 누락은 컴파일러가 잡아주지 않습니다.

이 챕터에서는 `RequireNestedClass()`와 `RequireNestedClassIfExists()`를 사용하여 중첩 클래스의 존재와 구조를 **컴파일 타임이 아닌 테스트 타임에** 자동으로 검증하는 방법을 학습합니다.
...
다음 표는 중첩 클래스 검증 메서드의 동작 차이를 비교합니다.
...
다음 표는 이 챕터에서 사용한 주요 필터와 검증 규칙을 정리합니다.
...
| `RequireImmutable()` | 중첩 클래스의 불변성 검증 |
...
**A**: Validator, Mapper, Profile처럼 모든 Command에 필수는 아니지만, 있을 경우 특정 규칙을 따라야 하는 선택적 중첩 클래스에 적합합니다. "있으면 규칙을 지켜라, 없어도 괜찮다"는 유연한 검증을 제공합니다.
