...
`Employee` 클래스가 `public sealed`인지 매번 코드 리뷰에서 눈으로 확인하고 있나요? 클래스가 10개일 때는 가능하지만, 50개, 100개로 늘어나면 놓치는 것은 시간 문제입니다. In this chapter, 이런 수동 확인을 **실행 가능한 테스트로** 바꾸는 방법을 배웁니다.
...
- **`private` 생성자 + 정적 factory method**: 객체 생성을 통제합니다.
...
**ClassValidator는** 플루언트 API를 제공하여 여러 규칙을 체이닝할 수 있습니다. `RequirePublic().RequireSealed()`는 하나의 검증 안에서 두 가지 조건을 모두 verifies.

## Summary at a Glance

The following table 이 장에서 사용한 핵심 구성요소를 요약합니다.
...
Next chapter에서는 `public`/`internal` 가시성, `sealed`/`abstract`/`static` 한정자, `record` 타입까지 다양한 클래스 속성을 검증하는 방법을 배웁니다.
