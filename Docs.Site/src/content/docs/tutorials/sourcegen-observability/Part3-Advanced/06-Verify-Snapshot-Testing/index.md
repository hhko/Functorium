...
**A**: 맞습니다. 그래서 스냅샷 테스트는 결정적 출력을 전제로 합니다. 타임스탬프, GUID, 환경 변수 등 비결정적 요소가 생성 코드에 포함되면 매 실행마다 `.received.txt`가 달라져 테스트가 지속적으로 실패합니다. 이것이 12장에서 다룬 결정적 출력 원칙이 스냅샷 테스트의 필수 전제인 이유입니다.
