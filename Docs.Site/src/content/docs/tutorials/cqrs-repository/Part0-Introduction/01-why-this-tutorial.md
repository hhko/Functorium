...
### Q3: `IRepository`와 `IQueryPort`를 분리하면 코드가 더 많아지지 않나요?
**A**: 초기에는 인터페이스와 DTO가 늘어나지만, 조회 조건이 추가될 때 Repository에 메서드를 추가하는 "메서드 폭발" 문제가 사라집니다. `IQueryPort`는 Specification 기반 동적 검색을 지원하므로 새 필터 조건을 추가해도 인터페이스 변경이 필요 없습니다.
