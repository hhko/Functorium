...
5. 여러 저장소를 주입받아 LINQ Join으로 다중 Aggregate를 조회할 수 있습니다
...
### 다중 Aggregate 조회 (LINQ Join)

FAQ Q2에서 "여러 저장소를 주입받아 LINQ Join으로 처리합니다"라고 답했습니다. 실제 구현을 살펴봅시다.

주문(Order)과 상품(Product) 정보를 합쳐서 `OrderSummaryDto`를 반환하는 Query Adapter입니다.
...
핵심은 `GetProjectedItems`에서 두 저장소를 LINQ `join`으로 합치는 것입니다. `Specification<Order>`로 Order를 먼저 필터링한 뒤, Product와 조인하여 양쪽 필드를 하나의 DTO로 프로젝션합니다.

이 패턴은 Dapper에서 SQL JOIN으로 자연스럽게 대응됩니다:
...
**A**: 여러 저장소를 생성자로 주입받아 LINQ `join`으로 처리합니다. 위 "다중 Aggregate 조회" 섹션의 `InMemoryOrderSummaryQuery`가 그 예시입니다. `Specification<Order>`로 Order를 필터링한 뒤 Product와 조인하면, InMemoryQueryBase의 Search/SearchByCursor/Stream이 모두 자동으로 동작합니다.
