...
Part 1~2에서 value object의 개념과 검증 패턴을 익혔습니다. 이제 매번 보일러플레이트를 반복하지 않고, 프레임워크 기본 클래스를 상속하여 value object를 빠르게 구현하는 방법을 학습합니다.

`SimpleValueObject<T>`는 value object 패턴의 가장 기본적인 형태로, immutability(Immutable), 값 기반 동등성(Value-based Equality), 타입 안전성(Type Safety)을 기본 클래스 상속만으로 확보합니다.

## Learning Objectives

- `SimpleValueObject<T>`를 상속하여 value object를 구현할 수 있습니다
- 값 기반 동등성(`==`, `!=`)과 해시코드(`GetHashCode()`)의 동작을 설명할 수 있습니다
- 명시적 타입 변환을 활용하여 내부 값을 추출할 수 있습니다
- value object의 immutability 보장 메커니즘을 이해하고 적용할 수 있습니다

## Why Is This Needed?

기본 타입으로 도메인 값을 표현하면 세 가지 실질적인 문제가 생깁니다.

사용자 ID와 주문 ID가 모두 `int`라면, `userId = orderId` 같은 할당이 컴파일을 통과합니다. 타입 시스템이 논리적 오류를 잡아주지 못하는 것입니다. 또한 기본 타입은 데이터를 저장할 뿐 유효성 검증이나 비즈니스 로직을 포함하지 않으므로, 관련 로직이 여러 곳에 분산되어 유지보수가 어려워집니다. 마지막으로, 기본 타입은 값이 언제든 변경될 수 있어 예상치 못한 side effect이 발생할 수 있습니다.

value object 패턴은 이 세 문제를 한꺼번에 resolves. 의미 있는 타입으로 compile time 안전성을 확보하고, 데이터와 validation logic을 한 곳에 캡슐화하며, 생성 후 값 변경을 원천 차단합니다.

## Core Concepts

### 값 기반 동등성

일반 클래스는 두 인스턴스의 메모리 주소가 다르면 다른 객체로 취급합니다. 반면 value object는 내부 값이 같으면 동일한 객체로 판단합니다.
...
value object는 생성 후 값을 변경할 수 없습니다. 생성자를 `private`으로 선언하고 정적 factory method를 통해서만 생성하도록 설계합니다. 이 구조 덕분에 여러 스레드가 동시에 value object를 읽어도 경쟁 상태가 발생하지 않습니다.
...
The following table `SimpleValueObject<T>` 기반 value object를 구현할 때 필수적인 네 가지 요소를 정리합니다.

| 포인트 | Description |
|--------|------|
| **`SimpleValueObject<T>` 상속** | 기본적인 value object 기능을 상속받습니다 |
| **private 생성자** | 외부에서 직접 생성하지 못하도록 제한합니다 |
| **정적 Create 메서드** | 유효성 검증과 객체 생성을 담당합니다 |
| **DomainError.For\<T>()** | 구조화된 error handling를 위한 정적 메서드입니다 |
...
`BinaryData`는 `SimpleValueObject<byte[]>`를 상속하여 이진 데이터를 value object로 표현합니다.

**BinaryData.cs - value object 구현**
```csharp
...
일반 클래스와 `SimpleValueObject<T>` 기반 value object의 차이를 compares.

### Comparison Table
| Aspect | 일반 클래스 | value object (`SimpleValueObject<T>`) |
|------|------------|-------------------------------|
| **동등성** | reference equality | value equality |
| **immutability** | 변경 가능 | 변경 불가능 |
...
### Q2: value object와 일반 클래스의 차이점은 무엇인가요?
**A**: 일반 클래스는 reference equality을 사용하고 값을 변경할 수 있습니다. value object는 내부 값이 같으면 동일한 객체로 취급하고, 생성 후 값 변경이 불가능합니다.

### Q3: 왜 비교 연산자를 지원하지 않나요?
**A**: 이진 데이터에서 "더 크다/작다"는 의미가 도메인마다 다르게 해석될 수 있어, 오히려 혼란을 유발합니다. 비교가 필요한 경우 `ComparableSimpleValueObject<T>`를 uses.

Next chapter에서는 `SimpleValueObject<T>`에 비교 기능을 추가한 `ComparableSimpleValueObject<T>`를 학습합니다. value object에 자연스러운 순서가 있을 때 정렬과 비교 연산을 어떻게 지원하는지 examines.
