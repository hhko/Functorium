...
주소 목록을 도시별로 정렬하거나, 두 주소의 순서를 비교해야 할 때 `ValueObject`만으로는 충분하지 않습니다. `ComparableValueObject`는 여러 value object를 조합하면서도 비교와 정렬 기능을 자동으로 제공하여, value object 패턴의 완성된 형태를 implements.

## Learning Objectives

1. `ComparableValueObject`를 상속하여 비교 가능한 복합 value object를 구현할 수 있습니다.
2. `GetComparableEqualityComponents()`를 오버라이드하여 의미 있는 비교 순서를 정의할 수 있습니다.
3. LINQ의 `OrderBy()`, `Where()` 등에서 복합 value object를 자연스럽게 사용할 수 있습니다.
4. 비교 연산자(`<`, `<=`, `>`, `>=`)가 자동으로 지원됨을 확인할 수 있습니다.

## Why Is This Needed?

Previous step인 `05-ValueObject-Composite`에서는 여러 value object들을 조합하여 복합적인 도메인 개념을 표현할 수 있었습니다. 하지만 이러한 복합 value object들을 정렬하거나 비교하려면 수동으로 비교 로직을 구현해야 했습니다.

주소나 복합 데이터들을 정렬할 때 기준이 되는 값이 무엇인지 명확하지 않고, `OrderBy()`, `Min()`, `Max()` 같은 LINQ 메서드에서 복합 value object를 직접 사용할 수 없었습니다. 비교 연산자도 지원되지 않아 두 주소의 순서를 직관적으로 비교할 방법이 없었습니다.

`ComparableValueObject`는 이 문제를 resolves. `GetComparableEqualityComponents()`에서 반환하는 `IComparable` 요소들의 순서로 자연스러운 정렬 기준을 정의하며, `IComparable<T>` 구현과 비교 연산자를 자동으로 provides.

## Core Concepts

### 완전한 value object 컴포지션

`ComparableValueObject`는 여러 개별 value object들을 조합하면서도 완전한 비교 기능을 provides. 주소는 도로명, 도시, 우편번호로 구성되며, 각 부분은 독립적인 value object이지만 전체 주소는 비교 가능한 하나의 단위로 작동합니다.
...
`GetComparableEqualityComponents()`를 통해 비교 순서를 명시적으로 defines. 주소의 경우 도시(가장 큰 지리적 단위)를 먼저 비교하고, 같은 도시라면 우편번호, 같은 우편번호라면 도로명으로 비교하는 것이 자연스럽습니다.
...
비교 가능한 복합 value object 구현의 핵심 네 가지입니다.
...
`Address`는 Street, City, PostalCode 세 value object를 조합하고, 비교 순서를 City -> PostalCode -> Street로 defines.

**Address.cs - 비교 가능한 복합 value object**
```csharp
...
**Program.cs - 완전한 value object 데모**
```csharp
// 비교 연산자 자연스러운 사용
...
`ValueObject` 기반 복합 value object와 `ComparableValueObject` 기반 복합 value object의 기능 차이를 compares.
...
| **비교 기능** | 미지원 | 자동 지원 |
| **LINQ 정렬** | 수동 구현 | 자동 지원 |
| **operator overloading** | 미지원 | 자동 지원 |
...
### Q3: 모든 복합 value object에 ComparableValueObject를 사용해야 하나요?
...
지금까지 프레임워크 기본 클래스를 활용한 value object 패턴을 모두 살펴보았습니다. Next chapter에서는 SmartEnum을 사용하여 도메인 로직을 내장한 타입 안전 enumeration을 구현하는 방법을 다룹니다.
