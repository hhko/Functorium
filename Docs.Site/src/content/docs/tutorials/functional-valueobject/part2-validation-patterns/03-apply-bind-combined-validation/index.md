...
The following code 단일 방식 처리와 2단계 혼합 처리를 compares.
...
OrderInfo value object는 Apply로 고객 정보를 병렬 검증하고, Bind로 금액 정보를 순차 검증합니다.
...
The following table 세 가지 검증 패턴의 특성을 compares.

| Aspect | Apply 병렬 검증 | Bind 순차 검증 | Apply+Bind 혼합 검증 |
|------|----------------|----------------|---------------------|
| **적용 대상** | 모든 검증이 독립적 | 모든 검증이 의존적 | 독립 + 의존 혼합 |
| **실행 방식** | 모든 검증을 병렬 실행 | 모든 검증을 순차 실행 | 단계별 최적화된 실행 |
| **error handling** | ManyErrors로 모든 에러 수집 | 단일 Error로 조기 중단 | 단계별 에러 구분 |
| **성능** | 병렬 실행으로 빠름 | 조기 중단으로 효율적 | 각 단계에서 최적화 |

The following table 혼합 검증 패턴의 장단점을 정리합니다.

| Pros | Cons |
|------|------|
| 복잡한 도메인 요구사항에 적합 | validation logic이 복잡해짐 |
...
**A:** 도메인 객체가 독립적인 정보와 의존적인 정보를 모두 포함할 때 uses. 주문 정보에서 고객 정보는 독립적이지만 금액 정보는 의존적인 경우가 대표적입니다.

### Q2: Apply와 Bind의 순서는 어떻게 결정하나요?
**A:** 일반적으로 독립적인 검증을 먼저 실행하고, 의존적인 검증을 나중에 실행합니다. 전제 조건이 되는 검증을 먼저 수행하고, 그 결과를 바탕으로 복잡한 business rule을 검증하면 불필요한 연산을 피할 수 있습니다.

### Q3: error handling는 어떻게 구분하나요?
**A:** Apply 단계에서는 ManyErrors 타입으로 여러 에러를 수집하고, Bind 단계에서는 단일 Error 타입으로 조기 중단합니다. 사용자에게는 어떤 단계에서 실패했는지 명확하게 알려주는 것이 중요합니다.

지금까지 Apply와 Bind를 하나의 흐름에서 순서대로 조합했습니다. 하지만 각 필드 내부에서도 복잡한 다단계 검증이 필요하다면 어떻게 해야 할까요? Next chapter에서는 Apply 내부에 Bind를 중첩하여 필드별 세밀한 검증을 implements.
