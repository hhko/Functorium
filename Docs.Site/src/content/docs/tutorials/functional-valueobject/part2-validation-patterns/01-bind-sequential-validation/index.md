...
Part 1에서 단일 value object의 유효성을 보장하는 방법을 다루었습니다. 하지만 실제 애플리케이션에서는 여러 필드를 동시에 검증해야 하고, 필드 간에 의존 관계가 존재하는 경우가 흔합니다. 예를 들어 주소를 검증할 때, 국가 코드가 "KR"이면 우편번호는 5자리여야 하고 "JP"이면 7자리여야 합니다. 이런 의존적 검증 규칙을 Bind 패턴으로 어떻게 체이닝하는지 examines.
...
복잡한 domain rule을 가진 value object를 구현하면 곧 세 가지 현실적 문제에 부딪힙니다.
...
검증 순서의 중요성도 간과할 수 없습니다. 기본 형식 검증이 먼저 이루어져야 복잡한 business rule 검증이 의미를 가집니다.

**Bind 순차 검증 패턴은** 이 세 가지 문제를 의존성 체인을 통해 한꺼번에 resolves.

## Core Concepts

### Bind 순차 실행 메커니즘

Bind는 이전 검증의 결과를 다음 검증의 입력으로 전달합니다. 각 단계가 성공해야만 Next step가 실행되므로, 첫 번째 실패 시점에서 체인이 즉시 중단됩니다.

The following code 모든 검증을 독립적으로 실행하는 방식과 Bind 체이닝 방식을 compares.
...
특정 조건이 만족되어야 Next step로 진행할 수 있는 business rule을 Bind로 자연스럽게 표현할 수 있습니다.
...
이 패턴을 사용하면 복잡한 business rule을 명확하고 타입 안전하게 표현할 수 있습니다.
...
Address value object는 Bind를 사용하여 도로명, 도시, 우편번호, 국가일치를 순차적으로 검증합니다.
...
The following table 독립 실행 방식과 Bind 순차 검증 방식의 차이를 compares.

| Aspect | Previous approach | Bind 순차 검증 |
|------|-----------|----------------|
| **실행 방식** | 모든 검증을 독립적으로 실행 | 순차적으로 체이닝하여 실행 |
| **의존성 처리** | 검증 규칙 간 의존성 고려 안함 | 이전 결과가 다음 검증에 전달 |
| **성능** | 모든 검증을 실행하여 비효율적 | 첫 번째 실패에서 조기 중단 |
| **business rule** | 단순한 개별 검증 | 복잡한 의존성 규칙 표현 가능 |

The following table Bind 순차 검증의 장단점을 정리합니다.

| Pros | Cons |
|------|------|
| 첫 번째 실패에서 즉시 중단하여 효율적 | 검증 순서를 신중히 설계해야 함 |
| 복잡한 business rule을 명확히 표현 | 모든 검증을 동시에 실행할 수 없음 |
| 검증 순서가 비즈니스 로직과 일치 | 중간 단계에서 실패 시 원인 파악 필요 |

## FAQ

### Q1: Bind와 Apply의 차이점은 무엇인가요?
**A:** Bind는 순차 실행과 조기 중단을 제공하고, Apply는 병렬 실행과 모든 에러 수집을 provides. 이전 결과가 다음 검증에 전달되어야 하는 의존성 검증에는 Bind가, 서로 독립적인 검증 규칙들을 동시에 실행할 때는 Apply가 적합합니다.

### Q2: 언제 Bind 패턴을 사용해야 하나요?
**A:** 검증 규칙들 간에 의존성이 있고, 특정 순서로 실행되어야 하는 경우에 uses. 주소 검증에서 도로명이 유효해야 도시 검증이 의미가 있고, 도시가 유효해야 우편번호 검증이 의미가 있는 경우가 대표적입니다.

### Q3: Map을 왜 사용하나요?
**A:** Bind 체인에서 중간 결과들을 무시하고 원본 매개변수로 최종 결과를 구성하기 위해서입니다. 검증 과정에서 변환된 값이 아닌 원본 입력값들을 사용하여 최종 객체를 생성하므로, 검증과 객체 생성이 깔끔하게 분리됩니다.

그러나 Bind는 한 번에 하나의 에러만 보고합니다. 사용자에게 모든 문제를 한꺼번에 알려주려면 어떻게 해야 할까요? Next chapter에서 독립적인 검증을 병렬로 실행하는 Apply 패턴을 examines.
