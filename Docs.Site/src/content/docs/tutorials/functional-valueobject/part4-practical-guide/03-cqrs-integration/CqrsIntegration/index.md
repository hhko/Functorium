...
CQRS(Command Query Responsibility Segregation) 패턴에서 value object를 활용하는 방법을 학습합니다.
...
**A**: Controller에서 Handler의 결과를 받아 `ToApiResponse()`를 호출하면, 성공 시 데이터가 포함된 200 응답을, 실패 시 error message가 포함된 응답을 일관된 형식으로 returns. 모든 엔드포인트에서 동일한 응답 구조를 유지할 수 있습니다.
