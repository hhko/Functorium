...
사용자 입력이 유효한지 확인만 하고 싶은데, 기존 `Create` 메서드는 검증과 객체 생성을 동시에 수행합니다. 검증만 필요한 상황에서도 불필요한 객체 생성 비용을 지불해야 하는 셈입니다. In this chapter, Create와 Validate의 책임을 명시적으로 분리하여 single responsibility principle을 적용하고, validation logic의 재사용성과 테스트 용이성을 확보합니다.
...
Previous step인 `ValueComparability`에서는 value object의 comparability을 구현하여 동등성과 정렬 기능을 제공했습니다. 하지만 복잡한 비즈니스 로직에서는 검증과 생성이 한 메서드에 묶여 있으면 문제가 됩니다.

Create 메서드에 validation logic이 포함되어 있으면, 검증만 필요한 상황(예: 폼 유효성 확인, API 요청 검증, 배치 데이터 필터링)에서도 불필요한 객체 생성 비용이 발생합니다. 또한 validation logic과 생성 로직이 혼재되어 각각을 독립적으로 테스트하기 어렵습니다.

Create와 Validate를 분리하면 validation logic의 재사용성, single responsibility principle 준수, 테스트 용이성을 모두 확보할 수 있습니다.

## Core Concepts

### 검증 책임 분리 (Validation Responsibility Separation)

validation logic을 독립적인 메서드로 분리하여 재사용 가능하게 만듭니다. Validate 메서드는 검증 결과만 반환하고, Create 메서드는 Validate에 검증을 위임한 뒤 함수형 합성으로 객체를 생성합니다.

The following code Previous approach과 Improved approach의 차이를 보여줍니다.
...
LanguageExt의 Validation과 Fin을 활용하면 검증 실패 시 예외가 아닌 타입 안전한 error handling를 구현할 수 있습니다. 검증 결과를 `Map`으로 변환하고 `ToFin()`으로 최종 타입을 맞추는 파이프라인입니다.
...
각 단계가 명확히 분리되어 디버깅이 쉽고, 실패 시 자동으로 에러가 전파됩니다.
...
1. **Validate 메서드 구현**: validation logic만 담당하는 pure function로 구현하여 재사용성 확보
...
The following table Previous approach과 Current approach의 차이를 compares.

| Aspect | Previous approach | Current approach |
|------|-----------|-----------|
| **validation logic** | Create 메서드 내부에 포함 | Validate 메서드로 분리 |
| **재사용성** | 검증만 필요한 경우에도 객체 생성 필요 | Validate 메서드만 호출 가능 |
| **책임 분리** | Create가 검증과 생성을 동시 담당 | 각 메서드가 하나의 책임만 담당 |
| **테스트 용이성** | validation logic 테스트 시 불필요한 객체 생성 | validation logic을 독립적으로 테스트 가능 |
...
**A**: `Validation<Error, int>`는 검증 결과를 나타내며 성공 시 원본 값(int)을 returns. `Fin<Denominator>`는 객체 생성 결과를 나타내며 성공 시 생성된 객체(Denominator)를 returns. Validate는 검증 레벨, Create는 생성 레벨에서 작동합니다.
...
Create와 Validate를 분리하여 단일 value object의 검증 재사용성을 확보했습니다. Next chapter에서는 여러 value object를 조합한 복합 객체에서 검증된 값으로 직접 생성하는 CreateFromValidated 패턴을 다룹니다.
