...
Address를 생성할 때 Street, City, PostalCode가 이미 검증된 상태라면, 다시 검증할 필요가 있을까요? In this chapter, 이미 검증된 값에 대한 불필요한 재검증을 방지하는 **CreateFromValidated 메서드를** 도입하고, 복합 value object에서 Create, Validate, CreateFromValidated 세 가지 메서드가 각각 어떤 역할을 담당하는지 구현한다.

## Learning Objectives

1. 이미 검증된 값으로 직접 객체를 생성하는 CreateFromValidated 메서드를 구현할 수 있습니다
2. 여러 value object의 검증 결과를 LanguageExt의 tuple Apply 패턴으로 조합할 수 있습니다
3. Create, Validate, CreateFromValidated 세 가지 메서드를 상황에 맞게 선택하여 성능과 안전성을 동시에 확보할 수 있습니다

## Why Is This Needed?

Previous step인 `CreateValidateSeparation`에서는 Create와 Validate의 책임을 분리했습니다. 하지만 복합 value object를 다루면 새로운 문제가 드러납니다.

복합 객체를 생성할 때 각 구성 요소가 이미 검증된 상태라면, Create 메서드가 다시 검증을 수행하는 것은 불필요한 연산입니다. 또한 여러 value object의 검증 결과를 조합해야 하므로, 모든 검증이 성공했을 때만 복합 객체를 생성하는 안전한 합성이 필요합니다.

이를 해결하기 위해 Create, Validate, CreateFromValidated 세 가지 메서드 패턴을 introduces. 상황에 맞는 최적의 생성 방법을 선택하여 성능과 안전성을 동시에 확보할 수 있습니다.
...
The following code Previous approach의 중복 검증 문제와 Improved approach을 보여줍니다.
...
The following table Previous approach(Create + Validate)과 Current approach(3가지 메서드 패턴)의 차이를 compares.
...
**A**: 값이 이미 검증된 것이 확실한 경우에만 uses. 대표적으로 데이터베이스에서 조회한 값(저장 시점에 검증 완료), 복합 value object의 Validate 내부에서 자식 객체 생성, 비즈니스 로직에서 검증 완료 후 객체 생성이 해당됩니다.

### Q2: Validate 메서드에서 왜 CreateFromValidated를 사용하나요?
**A**: 성능 최적화와 타입 일치를 위해서입니다. Validate 성공 시 반환 타입이 value object여야 하는데, Create를 호출하면 `Fin<T>`를 반환하므로 다시 `T`로 변환해야 합니다. CreateFromValidated는 이미 검증된 원시 값을 받아 바로 value object를 반환하므로 불필요한 재검증과 타입 변환을 모두 피할 수 있습니다.
...
3가지 메서드 패턴으로 복합 value object의 성능과 안전성을 모두 확보했습니다. Next chapter에서는 이 패턴들을 재사용 가능한 프레임워크 타입으로 추상화하는 방법을 다룹니다.
