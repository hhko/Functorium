...
- 명시적 operator overloading으로 `Denominator` 타입 간의 **다양한 연산을 구현할 수 있습니다.**
- LINQ 표현식을 통해 **복합 연산에서 에러 전파를 자동화할 수 있습니다.**

### **실습을 통해 확인할 내용**
- **LINQ 표현식 활용**: `from` 키워드로 복잡한 `Match` 체인 단순화
- **operator overloading 강화**: `Denominator` 타입 간의 다양한 연산자 지원
- **error handling 개선**: 암시적 변환 없이도 안전한 타입 변환과 연산 수행

## Why Is This Needed?

Previous step인 `05-Operator-Overloading`에서는 operator overloading을 통해 자연스러운 수학 연산을 구현했습니다. 하지만 실제로 복잡한 연산을 수행하려고 할 때 몇 가지 문제가 발생했습니다.

두 개의 분모를 사용해서 연쇄 연산을 수행하려면 `Match` 메서드를 중첩해야 합니다. 단계가 늘어날수록 코드가 안쪽으로 계속 깊어지면서 가독성이 급격히 떨어집니다. 각 단계에서 성공/실패를 확인하고 에러를 Next step로 전달하는 과정도 번거롭고 실수하기 쉬우며, 정작 수행하려는 연산의 의도가 중첩된 `Match` 호출에 묻혀버립니다.

**LINQ 표현식**을 도입하면 이 모든 문제가 해결됩니다. `from`/`select` 구문으로 코드를 평탄하게 작성하면서, 에러 전파는 프레임워크가 자동으로 처리합니다.

## Core Concepts

### LINQ 표현식을 통한 함수형 error handling

LINQ 표현식은 **모나드 체이닝(Monadic Chaining)을** 구현하는 C#의 기능입니다. `from` 키워드를 사용해서 여러 단계의 연산을 체이닝하면, 각 단계의 성공/실패 처리가 자동으로 이루어집니다.

Previous approach의 중첩 `Match`와 LINQ 표현식을 compares.
...
### operator overloading을 통한 명시적 타입 변환

이전에는 암시적 변환에 의존해서 타입 안전성 문제가 발생할 수 있었습니다. 이번에는 `Denominator` 타입끼리도 연산할 수 있도록 필요한 연산자를 명시적으로 defines.
...
예를 들어, 세 단계의 연산 중 두 번째에서 에러가 발생하면 세 번째 단계는 실행되지 않고, 두 번째 단계의 에러가 최종 결과로 전파됩니다. 이 덕분에 코드가 훨씬 단순해지고 error handling 로직의 반복이 사라집니다.
...
2. **operator overloading 확장**: `Denominator` 타입 간의 다양한 연산자 구현 (int/Denominator, Denominator/Denominator)
3. **error handling 패턴**: `Match` 메서드를 통한 성공/실패 케이스 처리
...
#### Denominator 클래스의 operator overloading
```csharp
...
단일 연산부터 복합 연산, error handling까지 LINQ 표현식의 활용 범위를 보여줍니다.
...
| **error handling 자동화** | 디버깅 시 추적 복잡성 |
| **타입 안전성 강화** | operator overloading 복잡성 |
...
**A**: 체이닝 중간에 실패가 발생하면 나머지 단계를 건너뛰고 최종 `Fin<T>` 결과에 에러가 반영됩니다. 원본 error message는 그대로 보존되므로, 최종 결과를 `Match`로 처리하면 됩니다.
...
### Q3: operator overloading을 많이 추가하면 성능에 영향을 주나요?
**A**: 성능 영향은 미미합니다. operator overloading은 컴파일러에 의해 메서드 호출로 변환되며, JIT 컴파일러가 인라인 최적화를 적용할 수 있으므로 일반 메서드 호출과 동일한 수준의 오버헤드만 발생합니다.

---

여기까지 Part 1의 기본 개념(유효성 보장, operator overloading, LINQ 표현식)을 모두 다루었습니다. 이 세 가지가 결합되어 "생성 시점에 유효성을 확보하고, 자연스럽게 연산하며, 에러를 자동으로 전파하는" value object의 기초가 완성되었습니다. Next chapter에서는 **value equality(Value Equality)을** 구현하여, 같은 값을 가진 두 객체를 동일하게 취급하는 방법을 학습합니다.

→ [7장: value equality](../07-Value-Equality/)
