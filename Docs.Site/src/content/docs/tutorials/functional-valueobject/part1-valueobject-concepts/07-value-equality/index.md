...
`Denominator(5)`와 `Denominator(5)`는 같은 객체일까요? C#의 기본 동작에서는 메모리 주소가 다르므로 `false`를 returns. 하지만 value object라면 내부 값이 같을 때 같은 객체로 취급되어야 합니다. In this chapter, `IEquatable<T>` 인터페이스를 통해 타입 안전한 값 기반 동등성을 구현하고, `HashSet<T>`이나 `Dictionary<TKey, TValue>` 같은 컬렉션에서도 올바르게 동작하도록 guarantees.

## Learning Objectives

1. `IEquatable<T>` 인터페이스를 활용하여 value object의 동등성을 올바르게 구현할 수 있습니다
2. reference equality과 value equality의 차이점을 이해하고 적절한 상황에서 올바른 동등성을 선택할 수 있습니다
3. `HashSet<T>`, `Dictionary<TKey, TValue>` 등 해시 기반 컬렉션에서 값 기반 동등성이 올바르게 동작하는 것을 확인할 수 있습니다

## Why Is This Needed?

Previous step인 `LinqExpression`에서는 모나드 체이닝을 통해 함수형 합성성을 구현했습니다. 하지만 value object들을 컬렉션에서 사용하거나 비교 연산을 수행할 때 문제가 드러납니다.

C#의 모든 참조 타입은 기본적으로 reference equality을 uses. 값이 같아도 다른 인스턴스라면 `false`를 반환하므로, `HashSet<T>`이나 `Dictionary<TKey, TValue>`에서 값이 같은 객체들이 중복 저장되거나 키 검색이 실패합니다. 이는 `GetHashCode()`와 `Equals()`가 일관되지 않게 구현되었기 때문입니다. 또한 `Object.Equals()`를 사용하면 값 타입에서 박싱/언박싱 오버헤드가 발생하여 대량 데이터 처리 시 성능이 저하됩니다.
...
두 객체의 내부 값이 같으면 동일한 것으로 취급하는 개념입니다. DDD에서 value object(Value Object) 패턴의 핵심 원칙이며, 동일한 입력에 대해 항상 동일한 결과를 guarantees.

The following code reference equality과 값 기반 동등성의 차이를 보여줍니다.
...
`IEquatable<T>`는 동등성 비교에서 타입 안전성을 provides. `Object.Equals(object?)`와 달리 박싱/언박싱 오버헤드가 없고, compile time에 타입 검사가 이루어집니다.
...
4. **operator overloading**: `==`와 `!=` 연산자를 통한 자연스러운 비교 구문 지원
...
#### Denominator value object (값 기반 동등성 구현)
```csharp
public sealed class Denominator : IEquatable<Denominator>
{
...
The following table reference equality과 값 기반 동등성의 차이를 compares.

| Aspect | reference equality | 값 기반 동등성 |
|------|-------------|----------------|
| **비교 기준** | 메모리 주소 | 내부 값 |
...
### Q2: reference equality과 value equality 중 어떤 것을 사용해야 하나요?
**A**: value object(Value Object)는 value equality을, 엔티티(Entity)는 reference equality(또는 식별자 기반 동등성)을 uses. value object는 immutability과 값 기반 비교가 특징이고, 엔티티는 고유한 식별자로 구분됩니다.
...
### Q3: null 체크는 왜 필요한가요?
**A**: null 참조와의 비교에서 `NullReferenceException`을 방지하기 위해서입니다. `==` 연산자에서 `ReferenceEquals`로 양쪽 다 null인 경우를 먼저 처리하고, 한쪽만 null인 경우 `false`를 returns.
...
value equality이 확보되면, Next chapter에서는 value object의 순서 비교를 구현하여 정렬과 범위 검증이 가능한 comparability(Comparability)을 다룹니다.

---

→ [8장: comparability](../08-Value-Comparability/)
