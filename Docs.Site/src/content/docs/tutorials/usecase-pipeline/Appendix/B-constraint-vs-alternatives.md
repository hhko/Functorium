...
제네릭 제약 외에 다른 대안은 없을까요? 이 부록에서는 Functorium이 선택한 **인터페이스 제약 조건 방식**과 다른 대안들을 비교합니다. 각 접근 방식의 장단점을 분석하여, 왜 인터페이스 계층 + 제네릭 제약이 최선의 선택인지 이해합니다.
...
다섯 가지 접근 방식을 핵심 기준별로 비교하면 다음과 같습니다.
...
`FinResponse<A>`의 Map, Bind 체인이 구현하는 Railway Oriented Programming 패턴과, Pipeline과 ROP의 관계를 살펴봅니다.

→ [부록 C: Railway Oriented Programming 참조](C-railway-oriented-programming.md)
