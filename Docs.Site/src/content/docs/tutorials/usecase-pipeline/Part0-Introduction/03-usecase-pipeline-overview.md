...
```mermaid
flowchart TB
  Request --> Metrics["Metrics Pipeline<br/>메트릭 수집"]
  Metrics --> Tracing["Tracing Pipeline<br/>분산 추적"]
  Tracing --> Logging["Logging Pipeline<br/>요청/응답 로깅"]
  Logging --> Validation["Validation Pipeline<br/>입력 유효성 검사"]
