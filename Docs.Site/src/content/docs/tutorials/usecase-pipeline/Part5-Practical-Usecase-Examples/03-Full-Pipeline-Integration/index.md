...
이 튜토리얼의 최종 목표는 7개 기본 Pipeline이 리플렉션 없이 타입 안전하게 동작하는 것이었습니다. 이 장에서는 Part 4에서 학습한 **7개의 기본 Pipeline을 모두 연결**하여 전체 요청 처리 흐름을 시뮬레이션합니다. 실제 Functorium에서는 Custom Pipeline 슬롯도 제공하여 사용자 정의 Pipeline을 Handler 바로 앞에 추가할 수 있습니다. 실제 Mediator Pipeline은 DI(의존성 주입)를 통해 자동으로 등록되지만, 여기서는 학습 목적으로 수동 호출하여 각 Pipeline의 역할과 실행 순서를 명확하게 이해합니다.
...
```
Metrics → Tracing → Logging → Validation → Exception → Transaction → Handler
                                                                        ↓
Metrics ← Tracing ← Logging ← Validation ← Exception ← Transaction ← 결과
```
...
1. 7개 기본 Pipeline의 실행 순서와 중첩 구조를 설명할 수 있다
