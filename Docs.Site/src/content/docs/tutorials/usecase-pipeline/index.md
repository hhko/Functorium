...
[Part 1] 제네릭 변성 기초
  ...
  4장: ISP + 변성 조합
       |
[Part 2] 문제 정의
  1장: Mediator Pipeline      ->  2장: Fin<T> 직접 사용 한계  ->  3장: IFinResponse 래퍼 한계
       |
  4장: 요구사항 정리
       |
[Part 3] IFinResponse 계층 설계
  1장: IFinResponse 마커      ->  2장: IFinResponse<out A>   ->  3장: IFinResponseFactory
       |
  4장: IFinResponseWithError  ->  5장: FinResponse<A> DU
       |
[Part 4] Pipeline 제약 패턴
  1장: Create-Only 제약       ->  2장: Read+Create 제약      ->  3장: Transaction/Caching
       |
  4장: Fin→FinResponse
       |
[Part 5] 실전 Usecase
  1장: Command Usecase        ->  2장: Query Usecase         ->  3장: 전체 통합
```
