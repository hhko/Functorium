...
값 객체를 **만드는 법(HOW)**을 익혔다면, 다음 질문은 **언제, 왜(WHEN/WHY)** 이런 타입 구조가 필요한가입니다. **타입 주도 도메인 설계는** Union types(sealed record 계층)과 상태 기계를 활용하여, 불법 상태가 표현 자체가 불가능한 도메인 모델을 설계합니다. Contact 도메인의 점진적 진화를 통해 타입이 도메인 통찰을 이끌어내는 과정을 체험합니다.
...
| 원시 타입이 도메인 불변식을 보장하지 못함 | 타입 주도 도메인 설계 | [타입으로 도메인 설계하기](./designing-with-types/) | 24개 |
...
- **값 객체 → 타입 도메인 설계:** 함수형 값 객체에서 익힌 SimpleValueObject, ValidationRules가 타입 주도 설계의 기반 도구로 활용됩니다.
- **타입 도메인 설계 → CQRS 리포지토리:** Union types와 상태 기계로 설계한 도메인 모델이 CQRS의 Aggregate에 적용됩니다.
...
| **형식** | 참조 문서 | 단계별 실습 |
| **구성** | 주제별 23개 문서 | 8개 독립 과정, 총 340+ 프로젝트 |
