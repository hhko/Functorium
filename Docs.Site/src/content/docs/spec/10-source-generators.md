...
| `LogEnricherGenerator` | _(자동 감지)_ | `IUsecaseLogEnricher` 구현체 |
| `DomainEventLogEnricherGenerator` | _(자동 감지)_ | `IDomainEventLogEnricher` 구현체 |
...
| `[LogEnricherIgnore]` | `Functorium.Applications.Usecases` | 클래스, 프로퍼티, 파라미터 | LogEnricher 생성에서 제외 |
| `[LogEnricherRoot]` | `Functorium.Applications.Observabilities` | 인터페이스, 프로퍼티, 파라미터 | ctx 루트 레벨로 승격 |
...
| `FUNCTORIUM002` | Warning | `LogEnricherGenerator`, `DomainEventLogEnricherGenerator` | ctx 필드 타입 충돌 (OpenSearch 매핑) |
| `FUNCTORIUM003` | Warning | `LogEnricherGenerator` | Request 타입 접근 불가 |
| `FUNCTORIUM004` | Warning | `DomainEventLogEnricherGenerator` | Event 타입 접근 불가 |
...
## LogEnricherGenerator

`ICommandRequest<TSuccess>` 또는 `IQueryRequest<TSuccess>`를 구현하는 `record`를 자동 감지하여 `IUsecaseLogEnricher` 구현체를 생성합니다. 별도의 트리거 어트리뷰트 없이 인터페이스 구현만으로 동작합니다.
...
3. `[LogEnricherIgnore]`가 클래스 레벨에 적용되지 않아야 합니다.
...
| 생성 타입 | 설명 |
|----------|------|
| `{ContainingTypes}{RequestTypeName}LogEnricher` | `partial class`, `IUsecaseLogEnricher<TRequest, FinResponse<TSuccess>>` 구현체 |

### 생성 코드 구조

```csharp
public partial class {ContainingTypes}{RequestTypeName}LogEnricher
    : IUsecaseLogEnricher<{RequestFullType}, FinResponse<{ResponseFullType}>>
{
    ...
    private static void PushRootCtx(...);  // [LogEnricherRoot] 속성이 있을 때만 생성
}
```
...
| `[LogEnricherRoot]` 적용 | `ctx.{snake_case_name}` | `ctx.customer_id` |
...
| **FUNCTORIUM003** | Warning | `'{RequestType}' implements ICommandRequest/IQueryRequest but LogEnricher cannot be generated because '{TypeName}' is {accessibility}.` -- `private`이나 `protected` 타입에는 Enricher를 생성할 수 없습니다. `[LogEnricherIgnore]`를 적용하여 경고를 억제하십시오. |

---

## DomainEventLogEnricherGenerator

`IDomainEventHandler<TEvent>`를 구현하는 클래스를 자동 감지하여 `TEvent`에 대한 `IDomainEventLogEnricher` 구현체를 생성합니다. 같은 이벤트 타입에 여러 Handler가 있어도 Enricher는 한 번만 생성됩니다.
...
4. `TEvent`에 `[LogEnricherIgnore]`가 클래스 레벨에 적용되지 않아야 합니다.
...
| 생성 타입 | 설명 |
|----------|------|
| `{ContainingTypes}{EventTypeName}LogEnricher` | `partial class`, `IDomainEventLogEnricher<TEvent>` 구현체 |

### 생성 코드 구조

```csharp
public partial class {ContainingTypes}{EventTypeName}LogEnricher
    : IDomainEventLogEnricher<{EventFullType}>
{
...
`LogEnricherGenerator`와 동일한 규칙을 따르며, 추가로 `IDomainEvent` 기본 속성(`OccurredAt`, `EventId`, `CorrelationId`, `CausationId`)은 자동 제외됩니다. `IValueObject` 또는 `IEntityId<T>` 구현체 프로퍼티는 `.ToString()` 호출로 keyword 변환됩니다.
...
| `[LogEnricherRoot]` 적용 | `ctx.{snake_case_name}` | `ctx.order_id` |

### 진단 코드

| 코드 | 심각도 | 메시지 |
|------|--------|--------|
| **FUNCTORIUM002** | Warning | ctx 필드 타입 충돌 (LogEnricherGenerator와 동일 ID 공유) |
| **FUNCTORIUM004** | Warning | `'{EventType}' implements IDomainEvent but DomainEventLogEnricher cannot be generated because '{TypeName}' is {accessibility}.` -- `private`이나 `protected` 이벤트 타입에는 Enricher를 생성할 수 없습니다. `[LogEnricherIgnore]`를 적용하여 경고를 억제하십시오. |
...
- [관찰 가능성 사양](./08-observability) -- `IUsecaseLogEnricher`, `IDomainEventLogEnricher` 인터페이스 정의
