...
내장 `UsecaseLoggingPipeline`이 Request/Response 로그 출력 시 Serilog `LogContext`에 커스텀 속성을 자동으로 Push합니다. `LogEnricherGenerator` Source Generator가 `ICommandRequest<T>` / `IQueryRequest<T>` 구현 Request에 대해 구현 코드를 **자동 생성**합니다.
...
- `EnrichResponseLog`: Response 로그 출력 전에 호출됩니다. Request와 Response 모두 전달되어 응답 기반 필드 추가가 가능합니다.
- DI에 등록되지 않으면 null-safe로 동작하여 기본 로깅만 수행됩니다.
- `[LogEnricherRoot]`: 인터페이스/속성에 적용 → `ctx.{field}` Root Level 승격 (교차 Usecase 검색 가능).
- `[LogEnricherIgnore]`: 클래스/속성에 적용 → Enricher 자동 생성에서 제외.

#### IDomainEventLogEnricher\<TEvent\>

DomainEvent Handler의 Request/내부/Response 로그에 비즈니스 컨텍스트 필드를 추가합니다. `ObservableDomainEventNotificationPublisher`가 `ResolveEnrichment()`로 DI에서 해석하여 `using` 스코프로 Handler 전체에 적용합니다.
...
- `DomainEventLogEnricherGenerator`가 `IDomainEventHandler<T>` 감지로 자동 생성.
- `[LogEnricherRoot]`, `[LogEnricherIgnore]`, `partial void OnEnrichLog()` 지원.
- DI 등록: `services.AddScoped<IDomainEventLogEnricher<TEvent>, TEnricher>()`.
...
- `SetStandardRequestTags`: 5개 표준 태그(`request.layer`, `request.category.name`, `request.category.type`, `request.handler.name`, `request.handler.method`)를 자동 설정합니다.
...
- `application.usecase.command.requests` +1 (request.handler.name=CreateProductCommand)
...
> Observability로 해결할 수 없는 프로세스 크래시 분석은 [22-crash-diagnostics.md](../guides/observability/22-crash-diagnostics)을 참고하세요.
...
| `request.handler.name` | Handler 클래스 이름 | `"CreateOrderCommandHandler"` |
| `request.handler.method` | Handler 메서드 이름 | `"Handle"`, `"GetById"` |
| `@request.params` | type-filtered 파라미터 복합 객체 | 파라미터 객체 |
...
| `@response.message` | 메서드 반환 값 (Adapter Debug) | 반환 값 |
...
#### 규칙 1: 동적 필드의 `count` → `_count`

동적 필드(파라미터, 결과)에서 컬렉션 크기를 나타낼 때는 `_count` 접미사를 사용합니다.

```
# 올바른 예시
request.params.orders_count      # 컬렉션 파라미터 크기

# 잘못된 예시
request.params.orders.count      # _count 사용
```

#### 규칙 2: 정적 필드의 `count` → `.count`

정적 필드(이벤트 관련)에서는 `.count` 형식을 유지합니다.

```
# 올바른 예시
request.event.count              # 이벤트 개수
response.event.success_count     # 성공한 이벤트 수 (형용사 조합)
response.event.failure_count     # 실패한 이벤트 수 (형용사 조합)
...
| 필드명 | 규칙 | 설명 |
|--------|------|------|
| `request.event.count` | 정적 `.count` | DomainEvent 배치 발행 시 이벤트 개수 |
| `request.params.{name}_count` | 동적 `_count` | 컬렉션 파라미터 크기 |
...
| `@request.params` | ✅ | - | - | type-filtered 파라미터 복합 객체 |
| `@request.message` | ✅ | - | - | 전체 파라미터 객체 (Debug) |
| `response.status` | ✅ | ✅ | ✅ | `"success"`, `"failure"` |
| `response.elapsed` | ✅ | - | ✅ | 처리 시간(초) |
| `@response.message` | ✅ | - | - | 메서드 반환 값 (Debug) |
| `error.type` | ✅ | ✅ | ✅ | `"expected"`, `"exceptional"`, `"aggregate"` |
| `error.code` | ✅ | ✅ | ✅ | 도메인 특화 오류 코드 |
| `@error` | ✅ | - | - | 구조화된 오류 객체 |
...
| `request.category.name` | ✅ | ✅ | ✅ | `"event"` (Publisher), `"usecase"` (Handler) |
| `request.category.type` | ✅ | ✅ | ✅ | `"event"` (Handler만) |
| `request.handler.name` | ✅ | ✅ | ✅ | Event/Aggregate 타입명 또는 Handler 클래스명 |
...
# 동적 필드 (Adapter 레이어)
@request.params = { "id": "12345", "items": [...] }
@request.message = { "id": "12345", "items": [...] }
@response.message = {...}
...
**해결:** 동적 필드(파라미터)는 `_count`(`request.params.{name}_count`), 정적 이벤트 필드는 `.count`(`request.event.count`), 형용사/명사 조합은 `_count`(`response.event.success_count`, `response.event.failure_count`)를 사용합니다.

## FAQ

### Q1. Usecase Pipeline과 Adapter Pipeline의 관측성 차이는?

Application Layer(Usecase)는 `IPipelineBehavior`로 구현되며 `request.category.type`(command/query)이 포함됩니다. Adapter Layer는 Source Generator가 코드를 자동 생성하며 `request.category.type`이 없는 대신 `@request.params`(복합 객체)가 Info/Debug 레벨에서 기록됩니다.

### Q2. DomainEvent의 Publisher와 Handler는 왜 다른 레이어로 분류되나요?

Publisher는 이벤트 발행이라는 인프라 관심사를 담당하므로 Adapter 레이어(`request.layer = "adapter"`, `request.category.name = "event"`)로 분류됩니다. Handler는 이벤트에 대한 비즈니스 로직을 처리하므로 Application 레이어(`request.layer = "application"`, `request.category.name = "usecase"`, `request.category.type = "event"`)로 분류됩니다.
...
`UsecaseLoggingPipeline`은 `IUsecaseLogEnricher<TRequest, TResponse>?`를 optional 의존성(`= null`)으로 주입받습니다. 등록하지 않으면 null-safe로 동작하여 기본 로깅만 수행됩니다. 별도의 설정이나 fallback이 필요하지 않습니다. `IDomainEventLogEnricher<TEvent>`도 동일한 null-safe 패턴입니다.

## 참고 문서

| 문서 | 설명 |
|------|------|
| [Logging 매뉴얼](../guides/observability/19-observability-logging) | 구조화된 로깅 상세 가이드 |
| [Metrics 매뉴얼](../guides/observability/20-observability-metrics) | 메트릭 수집 및 분석 가이드 |
| [Tracing 매뉴얼](../guides/observability/21-observability-tracing) | 분산 추적 상세 가이드 |
| [코드 명명 규칙](../guides/observability/18b-observability-naming) | Observability 코드 명명 규칙 |
| [Logger 메서드 명명 규칙](../guides/observability/18b-observability-naming) | Logger 메서드 네이밍 규칙 |
