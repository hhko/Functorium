...
| Collection (`List<T>`, `IReadOnlyList<T>` 등) | `long` | Element count value |
| 기타 복합 타입 (class, record, struct) | — (excluded) | 스칼라/Collection만 대상 |
...
| `[CtxIgnore]` | `class`, `property`, `parameter` | 모든 Pillar에서 excluded. `[CtxTarget]`보다 우선 |
...
```
[CtxIgnore]? → YES → 모든 Pillar excluded
    ↓ NO
...
DomainEvent Enricher는 `IDomainEvent` 인터페이스의 기본 속성을 자동으로 excluded합니다. These properties are already output as standard fields (such as `request.event.id`).
...
> Source Generator가 생성한 코드는 이미 `ctx.` 접두사를 포함하므로 Safe망 변환 대상이 아닙니다. 이 변환은 수동으로 `LogContext.PushProperty`를 호출한 경우에만 적용됩니다.
...
|----------|-------|------|------|
| `FUNCTORIUM002` | Warning | Different OpenSearch type groups assigned to same ctx field name | e.g., `ctx.customer_id`가 enricher Ais `keyword`, enricher Bis `long` |
...
| `ObservableDomainEventNotificationPublisher` | DomainEvent Handler | `IDomainEventCtxEnricher<TEvent>`를 Resolves at runtime then calls `Enrich` |
| `UsecaseMetricsPipeline` | Application Layer | `MetricsTagContext`에서 ctx.* MetricsTag를 읽어 기존 TagList에 merge |
...
| Field Name | Application Layer | Adapter 레이어 | Description |
|------------|-------------------|---------------|------|
| **Static Fields** | | | |
...
| Event | Log 수준 | Application Layer | Adapter 레이어 | Description |
|-------|-----------|-------------------|---------------|------|
...
| `request.handler.name` | Handler class name | Event/Aggregate 타입명 | Handler class name |
...
> DomainEvent Publisher는 Adapter 레이어로 분류되므로, Adapter 레이어와 동일한 Event ID for service identification.
...
> DomainEventHandler는 Application Layer의 usecase로 분류되므로, Application Layer와 동일한 Event ID for service identification.
...
| DomainEvent Handler | `INotificationPublisher` | [DomainEventHandlerLoggingStructureTests](../../Tests/Functorium.Tests.Unit/AdaptersTests/Observabilities/Events/DomainEventHandlerLoggingStructureTests.cs) | Application Layer 패턴 |
...
| Instrument | Application Layer | Adapter 레이어 | DomainEvent Publisher | DomainEvent Handler | Type | Unit |
|------------|-------------------|---------------|----------------------|--------------------|------|------|
...
> **DomainEvent Metrics에서 excluded되는 태그:**
> `request.event.count`, `response.event.success_count`, `response.event.failure_count`는 are not used as Metrics tags.
...
| DomainEvent Handler | `INotificationPublisher` + `IMeterFactory` | [DomainEventHandlerMetricsStructureTests](../../Tests/Functorium.Tests.Unit/AdaptersTests/Observabilities/Events/DomainEventHandlerMetricsStructureTests.cs) | Application Layer 패턴 |
...
| Property | Application Layer | Adapter 레이어 |
|----------|-------------------|---------------|
...
> **Span Name 형식 차이:** Application Layer는 `.{type}` 세그먼트(command/query/event)를 포함하지만, Adapter 레이어는 CQRS type 구분이 없으므로 `.{type}` 세그먼트가 생략됩니다.

#### Tag Structure

| Tag Key | Application Layer | Adapter 레이어 | Description |
|---------|-------------------|---------------|------|
| **Request Tags** | | | |
...
| DomainEvent Handler | `INotificationPublisher` + `ActivitySource.StartActivity()` | [DomainEventHandlerTracingStructureTests](../../Tests/Functorium.Tests.Unit/AdaptersTests/Observabilities/Events/DomainEventHandlerTracingStructureTests.cs) | Application Layer 패턴 |
...
`ObservableSignal` is a static API for developers to directly emit operational logs within Adapter implementation code. Observable 래퍼가 설정한 Common 컨텍스트(`request.layer`, `request.category.name`, `request.handler.name`, `request.handler.method`) is automatically included.
...
- **Metrics excluded**: Observable 래퍼가 request/response/duration 메트릭을 이미 자동 생성. 커스텀 메트릭은 `IMeterFactory` 직접 사용.
- **Debug에서 Tracing excluded**: 캐시 미스 등 고빈도 이벤트를 span에 넣으면 trace 노이즈.
...
| 2023 | `adapter.signal.error` | Error | 복구 Not allowed 실패 (재시도 소진, 서킷 오픈) |
...
3. Adapter 코드에서 `ObservableSignal.Debug/Warning/Error` 호출 시 `ObservableSignalScope.Current`에서 Common 필드 획득
...
- [Logging Guide](../guides/observability/19-observability-logging) — Structure화된 로깅 상세 가이드
