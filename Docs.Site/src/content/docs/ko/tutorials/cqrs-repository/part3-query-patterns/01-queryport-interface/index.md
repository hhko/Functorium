...
IQueryPort는 3가지 조회 메서드(페이지네이션 변형 + 스트리밍)와 2가지 read-side 집계 헬퍼(존재·건수)를 제공합니다 — 총 5개.
...
| `Exists` | `FinT<IO, bool>` | Read-side 존재 확인 (리포팅·대시보드·조기 shortcut) |
| `Count` | `FinT<IO, int>` | Read-side 건수 (리포팅·페이지네이션 메타데이터) |

> **Exists / Count가 `IRepository`와 `IQueryPort` 양쪽에 있는 이유**
> - `IRepository.Exists` / `Count` → **write-side 불변식 체크** (예: `Create` 전 이메일 중복 확인)
> - `IQueryPort.Exists` / `Count` → **read-side 리포팅** (예: 대시보드 "미결제 주문이 있는가?")
>
> 동일 Specification을 양쪽에서 재사용 가능합니다. 구분은 호출자의 의도와 각 측의 성능 특성 중 어느 쪽이 중요한지에 달려 있습니다.
