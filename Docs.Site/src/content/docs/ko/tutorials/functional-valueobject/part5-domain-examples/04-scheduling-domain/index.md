...
종료일이 시작일보다 이른 예약이 저장된다면? 두 회의가 같은 시간대에 겹치는 줄 모르고 확정된다면? "매월 31일" 반복 일정을 2월에 어떻게 처리해야 하는가? 일정/예약 시스템에서 시간 데이터를 원시 타입으로 다루면 이런 문제들이 런타임 버그로 이어집니다.

이 장에서는 캘린더와 예약 시스템에 필요한 4가지 핵심 개념을 값 객체로 구현하여, 범위 검증과 충돌 검사를 타입 수준에서 보장합니다.
...
종료일이 시작일보다 이른 잘못된 데이터가 저장될 수 있는데, DateRange와 TimeSlot은 생성 시점에 범위 유효성을 검증합니다. 두 예약이 겹치는지 판단하는 로직이 여러 곳에 흩어지면 버그가 발생하기 쉬운데, 값 객체에 Overlaps/Conflicts 메서드를 캡슐화하면 일관된 중복 검사를 보장합니다. "매월 31일"을 2월에 처리하거나 "매주 월/수/금"의 다음 발생일을 계산하는 복잡한 반복 로직도 RecurrenceRule이 캡슐화합니다.
...
겹침 검사(`Start <= other.End && End >= other.Start`)와 같은 범위 로직이 값 객체 내부에 구현되어 어디서든 일관되게 사용됩니다.
...
"매주 월/수/금"이나 "매월 15일" 같은 반복 규칙을 값 객체로 표현하고, `GetOccurrences()`로 다음 발생일들을 계산합니다.
...
public static Fin<TimeSlot> CreateCrossMidnight(TimeOnly start, TimeOnly end)
{
    // 자정을 넘는 경우 end < start
    if (start == end)
        return DomainError.For<TimeSlot>(new ZeroDuration(), $"{start}~{end}", "TimeSlot has zero duration");
...
Part 5에서 다양한 도메인의 값 객체 구현을 살펴보았습니다. 부록에서는 LanguageExt 타입 참조, 프레임워크 타입 선택 가이드, 용어집 등 학습에 필요한 참고 자료를 제공합니다.
