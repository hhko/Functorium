...
모든 값 객체가 Domain을 가져야 하는 것은 아닙니다. 단순한 값 객체는 복잡한 검증 로직이 없어 Domain이 불필요할 수 있습니다. `RequireNestedClassIfExists`는 Domain이 있는 값 객체에만 올바른 구조를 강제하고, 없는 값 객체는 검증을 건너뜁니다.
