...
> 프로젝트에서 Repository 패턴을 사용하는 경우 해당 규칙을 조정하세요. 상세: [09-domain-services.md](../domain/09-domain-services)
...
| Stateless (인스턴스 필드 없음) | 09 | ✅ | `RequireNoInstanceFields()` (순수 패턴에 적용) |
| `IObservablePort` 의존 금지 | 09 | ✅ | `RequireNoDependencyOn("IObservablePort")` |
| Public 메서드 `Fin` 반환 | 09 | ✅ | `RequireAllMethods(filter, m => m.RequireReturnTypeContaining("Fin"))` (순수 패턴에 적용; Evans 패턴은 `FinT` 반환) |
...
| 에러 코드 형식 (`Application.Usecase.Name`) | 08b | ❌ | 런타임 검증 필요 |
...
| 에러 코드 형식 (`Adapter.Adapter.Name`) | 08c | ❌ | 런타임 검증 필요 |
