...
> **핵심**: `EfCoreRepositoryBase`가 `IRepository`의 13개 메서드(CRUD 8개: `Create`, `GetById`, `Update`, `Delete`, `CreateRange`, `GetByIds`, `UpdateRange`, `DeleteRange` + Specification 5개: `Exists`, `Count`, `FindAllSatisfying`, `FindFirstSatisfying`, `DeleteBy`)를 모두 기본 구현하므로, 서브클래스는 `ToDomain()`/`ToModel()` 변환과 도메인 전용 메서드만 구현합니다. `IHasStringId` 인터페이스를 통해 모든 Model의 `Id` 속성이 `string` 타입임을 보장하며, `ReadQuery()`가 Include를 자동 적용하여 N+1 문제를 구조적으로 방지합니다.
...
`InMemoryRepositoryBase<TAggregate, TId>`는 `ConcurrentDictionary` 기반으로 `IRepository`의 13개 메서드(CRUD 8개 + Specification 5개)를 모두 기본 구현하는 프레임워크 베이스 클래스입니다.
서브클래스는 `Store` 프로퍼티만 제공하면 되며, Soft Delete 등 특수 로직이 필요한 메서드만 오버라이드합니다.

다음 코드에서 주목할 점은 베이스 클래스가 CRUD를 기본 제공하고, Soft Delete가 필요한 `GetById`와 `Delete`만 오버라이드하는 구조입니다.
    ...
    : EfCoreRepositoryBase<Product, ProductId, ProductModel>, IProductRepository
{
    ...
    // IRepository의 13개 메서드(CRUD 8개 + Specification 5개)는 EfCoreRepositoryBase가 기본 구현 제공
...
> **참조**: 파이프라인 패턴은 [11-usecases-and-cqrs.md §트랜잭션과 이벤트 발행](../application/11-usecases-and-cqrs#트랜잭션과-이벤트-발행-usecasetransactionpipeline)을 참조하세요.
