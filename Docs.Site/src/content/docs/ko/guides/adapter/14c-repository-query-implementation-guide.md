...
**Q: 왜 CRUD 연산은 단건↔벌크가 대칭인데 Specification 연산은 비대칭인가요?**
A: CRUD 8개는 단건과 벌크의 실행 경로가 동일합니다. Create/Update는 호출자가 이미 도메인 객체를 가지고 있으므로 `ToModel` → `DbSet.Add/Update`를 똑같이 타고, Delete는 ID 하나든 여럿이든 `ExecuteDeleteAsync`를 사용하며, Read는 `ReadQuery()`를 공유하면서 조건만 단수/복수로 다릅니다. 자세한 비교표는 섹션 2.7을 참조하세요.

Specification 5개(`Exists`, `Count`, `FindAllSatisfying`, `FindFirstSatisfying`, `DeleteBy`)는 의도적으로 **비대칭**입니다. 각각 답해야 할 질문이 본질적으로 다르기 때문입니다 — 존재·개수(`Exists`/`Count`는 `bool`/`int` 반환), 조회 형태(`FindAll`은 `Seq<T>`, `FindFirst`는 `Option<T>`), 벌크 삭제(`DeleteBy`는 `int`). 단건↔벌크 쌍이 아니라 Evans의 `selectSatisfying` 패턴이 요구하는 서로 다른 유스케이스의 묶음입니다.
