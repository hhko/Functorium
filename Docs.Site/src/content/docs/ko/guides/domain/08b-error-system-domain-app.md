...
### DomainErrorKind 범주 구조 및 전체 목록

다음 표는 `DomainErrorKind`의 범주별 분류와 각 에러 타입이 정의된 파일을 정리한 것입니다.

| 범주 | 파일 | 설명 |
|------|------|------|
| Presence | `DomainErrorKind.Presence.cs` | 값 존재 검증 |
| Length | `DomainErrorKind.Length.cs` | 문자열/컬렉션 길이 검증 |
| Format | `DomainErrorKind.Format.cs` | 형식 및 대소문자 검증 |
| DateTime | `DomainErrorKind.DateTime.cs` | 날짜 검증 |
| Numeric | `DomainErrorKind.Numeric.cs` | 숫자 값/범위 검증 |
| Range | `DomainErrorKind.Range.cs` | min/max 쌍 검증 |
| Existence | `DomainErrorKind.Existence.cs` | 존재 여부 검증 |
| Custom | `DomainErrorKind.Custom.cs` | 커스텀 에러 |
...
| 에러 타입 | 설명 | 사용 예시 |
|-----------|------|----------|
| `Custom` | 도메인 특화 에러 (abstract) | `sealed record AlreadyShipped : DomainErrorKind.Custom;` → `new AlreadyShipped()` |
...
| 에러 타입 | 설명 | 사용 예시 |
|-----------|------|----------|
| `Custom` | 애플리케이션 특화 에러 (abstract) | `sealed record PaymentDeclined : ApplicationErrorKind.Custom;` → `new PaymentDeclined()` |
...
**원인:** Custom 에러의 정의 위치가 잘못되었거나, `DomainErrorKind.Custom`을 상속하지 않았을 수 있습니다.
**해결:** Custom 에러는 반드시 해당 레이어의 `Custom` abstract record를 상속해야 합니다. 예: `public sealed record InsufficientStock : DomainErrorKind.Custom;`
