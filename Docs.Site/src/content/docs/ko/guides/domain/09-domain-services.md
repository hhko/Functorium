...
- `DomainError.For<OrderCreditCheckService>` — 에러 코드 자동 생성 (`Domain.OrderCreditCheckService.CreditLimitExceeded`)
...
`DomainError.For<{ServiceName}>(new {ErrorKind}(), currentValue, message)` 패턴을 사용합니다. 에러 코드는 `Domain.{ServiceName}.{ErrorKind}` 형태로 자동 생성됩니다. 두 패턴 모두 동일합니다.
