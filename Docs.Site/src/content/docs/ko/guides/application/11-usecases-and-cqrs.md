...
public sealed class {동사}{엔티티}{Command|Query}
{
    ...
    internal sealed class Usecase(...) : I{Command|Query}Usecase<Request, Response>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            // Application 에러: ApplicationError.For<{UsecaseName}>(new {ErrorKind}(), value, message) 사용
        }
    }
}
```
...
다음 코드에서 주목할 점은 `Validate()`로 모든 필드를 먼저 검증한 뒤, `Apply()`로 병렬 검증 결과를 병합하고, 이미 검증된 값을 `Unwrap()`로 안전하게 변환한다는 것입니다.
...
`ApplicationError.For<TUsecase>()` 메서드와 `ApplicationErrorKind` sealed record를 사용합니다:
...
| `Custom` | 커스텀 에러 (상속 정의) | `public sealed record PaymentDeclined : ApplicationErrorKind.Custom;` → `new PaymentDeclined()` |
...
- **표준화**: `ApplicationErrorKind`의 표준 에러 타입 활용
...
FluentValidation 검증 실패는 `UsecaseValidationPipeline`에서 `AdapterErrorKind.PipelineValidation`으로 변환됩니다. 이는 Application 레이어의 `ApplicationErrorKind.ValidationFailed`와는 다른 에러 타입입니다:

| 검증 레이어 | 에러 타입 | 사용 위치 |
|------------|----------|----------|
| FluentValidation (Pipeline) | `AdapterErrorKind.PipelineValidation(PropertyName)` | `UsecaseValidationPipeline` 자동 처리 |
| VO/비즈니스 규칙 (Usecase) | `ApplicationErrorKind.ValidationFailed(PropertyName)` | Usecase 내 수동 사용 |
...
**A:** `ApplicationError.For<TUsecase>(ApplicationErrorKind, value, message)` 패턴을 사용합니다. 별도 클래스 정의 없이 인라인으로 사용합니다. 에러 코드는 자동으로 `Application.{UsecaseName}.{ErrorTypeName}` 형식으로 생성됩니다.
