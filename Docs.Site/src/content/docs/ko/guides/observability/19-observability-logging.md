...
> **핵심 원칙:** Functorium은 로그를 `request.*`, `response.*`, `error.*` 필드로 구조화하여, `error.type`으로 Expected/Exceptional을 자동 분류하고 적절한 Log Level을 선택합니다. 개발자는 별도의 로깅 코드 없이 파이프라인이 일관된 형식의 로그를 생성합니다.
...
| `@error.ErrorKind` | "ExpectedError" | 상세 분석용 (실제 클래스명) |

`error.type`은 항상 세 가지 값 중 하나이므로 쿼리와 필터링에 적합합니다. `@error.ErrorKind`은 실제 에러 클래스 이름을 포함하여 더 상세한 분석에 사용됩니다.
...
| `request.handler.name` | 핸들러명 | 핸들러명 | 핸들러명 |
...
**위치**: `Functorium.Applications.Usecases.CtxIgnoreAttribute`
...
**Information 레벨:**
- 기본적인 요청/응답 정보와 `@request.params` (type-filtered 파라미터 복합 객체) 포함
- 결과 값은 포함하지 않음
- 운영 환경에서 항상 활성화
...
| `request.category.name` | 카테고리명 | 카테고리명 | 카테고리명 |
| `request.handler.name` | 핸들러명 | 핸들러명 | 핸들러명 |
| `request.handler.method` | 메서드명 | 메서드명 | 메서드명 |
| `@request.params` | 파라미터 객체 (Info/Debug) | - | - |
| `@request.message` | 파라미터 객체 (Debug) | - | - |
| `response.status` | - | "success" | "failure" |
| `response.elapsed` | - | 처리 시간 | 처리 시간 |
| `@response.message` | - | 결과값 (Debug) | - |
| `error.type` | - | - | 에러 타입 |
| `error.code` | - | - | 에러 코드 |
| `@error` | - | - | 에러 객체 |
