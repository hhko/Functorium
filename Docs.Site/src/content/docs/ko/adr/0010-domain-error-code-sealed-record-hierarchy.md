...
- **DomainErrorKind**: `NotFound`, `InvalidState`, `InvalidTransition`, `DuplicateValue` 등 27종의 도메인 에러를 sealed record로 정의합니다. 27종은 실제 비즈니스 시나리오에서 반복 등장하는 도메인 에러 패턴을 정리한 결과입니다.
- **ApplicationErrorKind**: `Unauthorized`, `Forbidden`, `Conflict` 등 애플리케이션 레이어 에러를 정의합니다.
- **AdapterErrorKind**: `ExternalServiceFailure`, `DatabaseError` 등 어댑터 레이어 에러를 정의합니다.
...
- <span class="adr-good">Good</span>, because `DomainErrorKind.NotFound`를 사용하면 `"Notfound"` 같은 오타가 컴파일 오류로 즉시 발견되어 런타임 매칭 실패가 원천 차단됩니다.
...
- <span class="adr-bad">Bad</span>, because 레이어별 sealed record 계층(DomainErrorKind 27종 + ApplicationErrorKind + AdapterErrorKind)의 초기 설계와 분류 작업에 상당한 투자가 필요합니다.
...
- <span class="adr-good">Good</span>, because `DomainErrorKind.NotFound`처럼 타입으로 에러를 표현하므로 오타, 대소문자 불일치가 컴파일 타임에 차단됩니다.
...
- <span class="adr-bad">Bad</span>, because 27종의 DomainErrorKind + ApplicationErrorKind + AdapterErrorKind 계층을 초기에 설계하고, 새로운 에러 패턴 등장 시 계층을 확장해야 하는 유지보수 비용이 있습니다.
...
### 단일 ErrorKind (레이어 구분 없음)

- <span class="adr-good">Good</span>, because 모든 에러가 하나의 `ErrorKind` 계층에 속하여 구조가 단순하고 학습 비용이 낮습니다.
