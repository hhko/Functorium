...
Value Object 검증 결과를 FinT 체인에 합류시키는 두 가지 방법이 있습니다. 상황에 따라 선택합니다.

| 기준 | ApplyT 패턴 | Unwrap(수동 분기) 패턴 |
|------|------------|----------------------|
| VO 검증 실패 시 | 에러를 누적 수집하여 한 번에 반환 | 첫 실패 시점에서 즉시 반환 |
| VO 개수 | 2개 이상일 때 효과적 | 1개이거나 순차 의존이 있을 때 |
| 코드 형태 | `(v1, v2).Apply((a, b) => ...).As().ToFin()` | `if (createData.IsFail) return ...;` |
| 사용 장면 | Create 계열 (여러 필드 동시 검증) | Update 계열 (단일 필드 변경) |

**ApplyT 패턴** -- 병렬 검증, 에러 누적:
