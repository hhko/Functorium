...
| `ErrorCodeExceptionalAssertions` | `Assertions.Errors` | 예외 기반 에러 검증 확장 메서드 |
...
| `AdapterErrorAssertions` | `TAdapter` (어댑터 타입) | `AdapterErrors.{Name}.{ErrorName}` |
...
`DomainErrorType` 등에 의존하지 않는 범용 에러 코드 검증입니다.
...
public static class ErrorCodeAssertions
{
    ...
    // ErrorCodeExpected 변형 (0~3개 값 오버로드 + predicate)
...
예외 기반 에러(`ErrorCodeExceptional`) 검증을 위한 특화된 확장 메서드입니다.
