...
|----------|----------|----------|----------|
| 8. 인증된 이메일 재인증 | `InvalidTransition(FromState, ToState)` | `DomainErrorKind` (내장) | `Domain.EmailVerificationState.InvalidTransition` |
| 9. 삭제된 연락처 수정 | `AlreadyDeleted` | `Contact` (Aggregate 내 커스텀) | `Domain.Contact.AlreadyDeleted` |
| 10. 중복 이메일 등록 | `EmailAlreadyInUse` | `ContactEmailCheckService` (서비스 내 커스텀) | `Domain.ContactEmailCheckService.EmailAlreadyInUse` |
...
| 구조적 에러 타입 | `DomainErrorKind` + `DomainError.For<T>()` | 문자열 메시지 대신 `sealed record` 에러 타입으로 실패 원인을 식별, 에러 코드 자동 생성 |
