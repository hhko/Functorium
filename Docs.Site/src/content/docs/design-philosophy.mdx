...
모든 유스케이스는 다음 순서의 Pipeline을 자동으로 통과합니다.

> **Metrics → Tracing → Logging → Validation → Caching → Exception → Transaction → Usecase**

Pipeline은 Metrics → Tracing → Logging → Validation → Caching → Exception → Transaction 순서로 실행됩니다. 이 순서에는 이유가 있습니다:
...
4. **Caching은** Exception과 Transaction 전에 위치하여, 캐시 히트 시 트랜잭션 없이 즉시 반환합니다.
5. **Exception은** Transaction 바로 앞에서 예외를 구조화된 에러로 변환하여, 트랜잭션 롤백을 깔끔하게 처리합니다.
6. **Transaction이** 가장 안쪽에서 실행되어, 실제 비즈니스 로직만을 트랜잭션 경계로 감쌉니다.

## Observability by Design

**원칙:** 운영 안정성은 배포 이후가 아니라 설계 단계에서 고려됩니다.

**왜 필요한가:** 관측성을 사후에 추가하면 일관성이 깨지고 누락이 발생합니다. 장애 발생 시 "이 유스케이스에는 로그가 없다"는 상황을 피하려면, 모든 유스케이스에 자동으로 계측이 적용되어야 합니다.

**Functorium 구현 방식:** `IObservablePort`로 관측 가능한 포트를 추상화하고, `[GenerateObservablePort]`로 Observable wrapper를 자동 생성하며, Usecase Pipeline으로 횡단 관심사를 처리합니다.

### IObservablePort — 관측 가능한 포트 추상화
...
### `[GenerateObservablePort]` — Source Generator 자동 생성

Source Generator가 Adapter에 대한 Observable wrapper를 자동 생성하여, OpenTelemetry 기반의 Tracing/Logging/Metrics를 투명하게 제공합니다. 개발자가 관측 코드를 직접 작성할 필요가 없습니다.
...
### Usecase Pipeline — 횡단 관심사 자동 처리

Functorium은 두 가지 경로로 관측성을 자동화합니다.

| 관측 경로 | 대상 | 메커니즘 |
|----------|------|---------|
| **외부 입출력** | 모든 Command/Query | Mediator `IPipelineBehavior` |
| **내부 입출력** | Repository, QueryAdapter | `[GenerateObservablePort]` Source Generator |
...
| `UsecaseCachingPipeline` | ICacheable 요청 캐싱 |
