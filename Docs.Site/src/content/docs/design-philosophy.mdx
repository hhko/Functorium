...
`CtxEnricher` Source Generator는 Request/Response 속성을 `ctx.{snake_case}` 필드로 자동 변환하여, Logging과 Tracing에 비즈니스 컨텍스트를 전파합니다. `[CtxTarget]` 어트리뷰트로 Logging, Tracing, Metrics 중 어떤 Pillar에 전파할지 세밀하게 제어할 수 있습니다.
...
| `CtxEnricherGenerator` | ICommandRequest/IQueryRequest 구현 | `{Request}CtxEnricher` (LogContext 자동 enrichment) |
| `DomainEventCtxEnricherGenerator` | IDomainEventHandler 구현 | `{Event}CtxEnricher` (이벤트 컨텍스트 enrichment) |
