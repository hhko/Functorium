...
C#의 `enum`은 상태를 정의할 수 있지만, 상태 전이 규칙을 컴파일 타임에 강제할 수 없다. `switch` 문에서 새로운 상태를 추가해도 누락된 분기를 컴파일러가 경고하지 않으며, 잘못된 상태 전이(예: Shipped에서 Draft로)를 런타임에서야 발견하게 된다.

도메인의 상태 전이 규칙을 타입 시스템으로 표현하여 컴파일 타임에 exhaustive check를 강제하고, 잘못된 전이를 원천 차단할 수 있는 상태 머신 패턴이 필요하다.
...
각 상태를 `UnionValueObject<TSelf>`를 상속하는 sealed record로 정의하고, `[UnionType]` 어트리뷰트를 부착하면 Source Generator가 exhaustive `Match`/`Switch` 메서드를 자동 생성한다.

- **Match**: 모든 상태에 대한 함수를 인자로 받아 결과를 반환. 하나라도 빠지면 컴파일 에러.
- **Switch**: Match와 동일하나 반환값 없이 부수 효과만 수행.
- **상태 전이**: 각 상태 타입에 `TransitionTo()` 메서드를 정의하여 허용된 전이만 타입 수준에서 표현. 허용되지 않은 전이는 `DomainErrorType.InvalidTransition` 반환.

Value Object 기반이므로 불변성이 보장되고, LINQ 등 컬렉션 연산과 자연스럽게 결합된다.

### 결과

- **긍정적**: 새 상태 추가 시 모든 Match/Switch 호출부에서 컴파일 에러가 발생하여 누락을 방지한다. 상태 전이 규칙이 타입에 인코딩되어 코드가 곧 문서다. Value Object의 불변성과 동등성 비교를 그대로 활용한다. Source Generator로 런타임 오버헤드가 없다.
- **부정적**: Source Generator의 빌드 의존성이 추가된다. Union 타입 패턴에 익숙하지 않은 C# 개발자에게 학습 비용이 있다. IDE 지원(자동완성, 리팩토링)이 Source Generator에 의존한다.
