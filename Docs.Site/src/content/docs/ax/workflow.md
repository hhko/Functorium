---
title: "워크플로"
description: "PRD에서 테스트까지 6단계 개발 워크플로"
---

AX는 PRD 작성부터 테스트까지 6단계로 이어지는 개발 워크플로를 제공합니다. 각 단계는 이전 단계의 산출물을 입력으로 받아, 설계 의도가 코드까지 일관되게 전달됩니다.
...
```
 ...
 5. adapter-develop       연결 -> Repository, Endpoint, DI 구현
        |
        v
 6. test-develop          검증 -> 단위/통합/아키텍처 테스트
```
...
### 6단계: Test Develop -- 품질 검증

| 항목 | 내용 |
|------|------|
| 스킬 | [test-develop](./skills/test-develop/) |
| 입력 | 구현된 소스 코드, `03-implementation-results.md` 문서들 |
| 출력 | 단위 테스트, 통합 테스트, 아키텍처 규칙 테스트 코드 |
| 핵심 활동 | VO/Aggregate/Usecase 단위 테스트, HostTestFixture 통합 테스트, ArchUnitNET 규칙 |

구현이 설계 의도를 충족하는지 검증합니다. Value Object의 Create 성공/실패, Aggregate의 상태 변경과 이벤트 발행, Usecase의 성공/실패 시나리오, HTTP 엔드포인트의 상태 코드, 레이어 의존성 방향을 테스트합니다.
