...
`observability-develop` 스킬에서 설계한 관측성 전략의 정합성을 테스트로 검증합니다.

**ctx.* 3-Pillar CtxPillar 스냅샷 테스트:**
- `[CtxTarget(CtxPillar.All)]`로 지정된 필드가 Logging/Tracing/MetricsTag 3곳 모두에 전파되는지 검증
- `[CtxTarget(CtxPillar.Logging)]` 전용 필드가 Tracing/Metrics에 누출되지 않는지 검증
- `[CtxIgnore]` 필드가 모든 Pillar에서 제외되는지 검증
- `[CtxRoot]` 필드가 `ctx.{field}` 루트 레벨로 승격되는지 검증

**메트릭 기준선 비교 테스트:**
- Counter/Histogram instrument가 올바른 Meter Name(`{service.namespace}.{layer}[.{category}]`)으로 등록되는지 검증
- 필드/태그 일관성: `request.layer`, `request.category.name`, `request.handler.name`, `response.status`, `error.type`, `error.code`가 Logging/Metrics/Tracing에서 동일 값인지 검증
- `response.elapsed`가 Metrics Tag가 아닌 Histogram instrument로 기록되는지 검증

**ObservableSignal 테스트:**
- Debug/Warning/Error 레벨별 Pillar 범위 검증 (Debug → Logging만, Warning/Error → Logging + Activity Event)
- 부가 필드 프리픽스(`adapter.*`) 네이밍 규칙 검증
