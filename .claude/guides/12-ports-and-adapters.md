...
│
├── IRepository<TAggregate, TId> : IAdapter   ← Aggregate Root 단위 Repository
...
│
└── IQueryAdapter<TEntity, TDto> : IAdapter   ← 읽기 전용 조회 (DTO 직접 반환)
    └── FinT<IO, PagedResult<TDto>> Search(Specification<TEntity>?, PageRequest, SortExpression)
    │
    ├── IProductQueryAdapter : IQueryAdapter<Product, ProductSummaryDto>
    ├── IProductWithStockQueryAdapter : IQueryAdapter<Product, ProductWithStockDto>  ← JOIN 예제
    └── IInventoryQueryAdapter : IQueryAdapter<Inventory, InventorySummaryDto>
```
...
- [ ] `IAdapter` 인터페이스를 상속하는가? (Repository인 경우 `IRepository<TAggregate, TId>` 상속)
...
프레임워크가 제공하는 `IQueryAdapter<TEntity, TDto>` 제네릭 인터페이스를 상속하여 정의합니다.
...
**핵심 포인트**:
- `IQueryAdapter<TEntity, TDto>`를 상속 — Search 시그니처가 자동 제공됨
...
| `IAdapter` 상속 | `IRepository<T, TId>` | `IAdapter` | `IAdapter` | `IQueryAdapter<TEntity, TDto>` |
...
appsettings.json에서 Options 클래스의 `SectionName` 상수값과 일치하는 섹션을 정의합니다. 환경별 오버라이드는 [ASP.NET Core Configuration](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration) 문서를 참조하세요. 통합 테스트 appsettings 설정은 [14-testing-library.md](./14-testing-library.md)를 참조하세요.
