...
```
              │ Products  │ Inventories │ Orders  │ Customers │ SharedKernel │
──────────────┼───────────┼─────────────┼─────────┼───────────┼──────────────┤
...
| **SharedKernel** | `SharedKernel/` (공유 VO, Entity, Event) | — | — |
...
- 2개 이상 Aggregate에서 공유하는 타입 → `SharedKernel/`
...
> 처음에는 Aggregate 전용으로 배치하고, 공유가 필요해지면 SharedKernel로 이동합니다. 이 규칙의 상세 판단 기준은 [01-project-structure.md FAQ §3](./01-project-structure.md)을 참조하세요.
...
| 금액 | 통화 + 수치 조합 | `Money` (Value Object) | SharedKernel |
| 수량 | 0 이상의 정수 값 | `Quantity` (Value Object) | SharedKernel |
...
## 8. Bounded Context와 Context Map
...
| 패턴 | 설명 | Functorium 매핑 |
|------|------|----------------|
| **Shared Kernel** | 두 BC가 공유하는 도메인 모델 부분집합 | `SharedKernel/` 폴더 (`Money`, `Quantity`) |
...
**Shared Kernel → `SharedKernel/ValueObjects/`**

`Money`, `Quantity` 등 여러 Module이 공유하는 Value Object가 `SharedKernel/` 폴더에 배치되어 있습니다. 서비스 분리 시 NuGet 패키지로 추출하거나 각 BC에 복제하는 결정이 필요합니다.
