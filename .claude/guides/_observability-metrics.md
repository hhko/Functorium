...
Application Layer에서는 CQRS 타입에 따라 다른 Instrument를 사용하므로, `request.handler.cqrs`도 태그로 포함됩니다.
...
1. **고정된 태그 값 사용**: `request.layer`, `request.category`, `request.handler.cqrs`는 제한된 값만 가집니다.
...
- request.handler.cqrs: 2 (command, query)
