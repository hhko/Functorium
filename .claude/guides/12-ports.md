...
- [왜 Port-Adapter 아키텍처인가 (WHY)](#왜-port-adapter-아키텍처인가-why)
...
public interface IProductRepository : IPort  // ← Port
{
...
**위치**: `Functorium.Domains.Ports.IPort`
...
│
├── IRepository<TAggregate, TId> : IPort   ← Aggregate Root 단위 Repository
...
│   │   ├── FinT<IO, Option<Product>> GetByName(ProductName name)  ← 도메인 전용
...
│
├── IUnitOfWork : IPort   ← Application Layer의 트랜잭션 커밋 Port
│   └── FinT<IO, Unit> SaveChanges(CancellationToken)
│
├── IOrderMessaging : IPort
...
│
├── IExternalApiService : IPort
│   └── FinT<IO, Response> CallApiAsync(Request request, CancellationToken ct)
│
└── IQueryPort<TEntity, TDto> : IPort   ← 읽기 전용 조회 (DTO 직접 반환)
    └── FinT<IO, PagedResult<TDto>> Search(Specification<TEntity>?, PageRequest, SortExpression)
    │
    ├── IProductQueryAdapter : IQueryPort<Product, ProductSummaryDto>
    ├── IProductWithStockQueryAdapter : IQueryPort<Product, ProductWithStockDto>  ← JOIN 예제
    └── IInventoryQueryAdapter : IQueryPort<Inventory, InventorySummaryDto>
```

**계층 이해하기:**

- **IPort**: 모든 Adapter가 구현하는 기반 인터페이스. `RequestCategory` 속성 제공
- **IRepository\<TAggregate, TId\>**: Aggregate Root 단위 Repository의 공통 인터페이스. `AggregateRoot<TId>` 제약으로 컴파일 타임에 Aggregate 단위 영속화를 강제. Domain Layer에 위치 (`Functorium.Domains.Repositories`)
...
- [ ] `IPort` 인터페이스를 상속하는가? (Repository인 경우 `IRepository<TAggregate, TId>` 상속)
...
using Functorium.Domains.Ports;  // IPort
...
```csharp
// 프레임워크 제네릭 인터페이스 (Functorium.Applications.Queries)
public interface IQueryPort<TEntity, TDto> : IQueryPort
{
    FinT<IO, PagedResult<TDto>> Search(
        Specification<TEntity>? spec,  // null이면 전체 조회
        PageRequest page,
        SortExpression sort);
}
```
...
```csharp
// 단일 테이블 — 파일: {Application}/Usecases/Products/Ports/IProductQueryAdapter.cs
public interface IProductQueryAdapter : IQueryPort<Product, ProductSummaryDto> { }

// JOIN (Product + Inventory) — 파일: {Application}/Usecases/Products/Ports/IProductWithStockQueryAdapter.cs
public interface IProductWithStockQueryAdapter : IQueryPort<Product, ProductWithStockDto> { }
```

> **참조**: `Tests.Hosts/01-SingleHost/Src/LayeredArch.Application/Usecases/Products/Ports/IProductQueryAdapter.cs`
...
| `IPort` 상속 | `IRepository<T, TId>` | `IPort` | `IPort` | `IQueryPort<TEntity, TDto>` |
...
Usecase의 Request/Response 패턴과 동일한 이름 패턴을 IPort Port 인터페이스에 적용하여 개념을 단순화하고, 레이어 간 데이터 변환 책임을 명확히 정의합니다.

#### Usecase와 Port의 Request/Response 차이점

| 관점 | Usecase Request/Response | Port Request/Response |
|------|--------------------------|----------------------|
| **위치** | Command/Query 클래스 내부 | IPort 인터페이스 내부 |
...
단일 메서드 인터페이스:
```csharp
public interface IWeatherApiService : IPort
{
...
```csharp
public interface IEquipmentApiService : IPort
{
...
| [14-adapter-wiring.md](./14-adapter-wiring.md) | Pipeline 생성, DI 등록, 테스트 |
