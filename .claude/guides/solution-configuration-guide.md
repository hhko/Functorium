...
프로젝트 수준 파일(`AssemblyReference.cs`, `Using.cs`)은 [service-project-structure-guide.md](./service-project-structure-guide.md)를 참조하세요.
...
| ORM | 데이터 액세스 | `Microsoft.EntityFrameworkCore.*` |
...
```
{SolutionDir}/.coverage/reports/    ← HTML 리포트, 병합 커버리지
{SolutionDir}/.nupkg/               ← NuGet 패키지 (.nupkg, .snupkg)
```
...
- [service-project-structure-guide.md](./service-project-structure-guide.md) — 프로젝트 수준 구성 (폴더, 네이밍, 의존성)
- [unit-testing-guide.md](./unit-testing-guide.md) — 테스트 작성 방법론 (MTP 설정 포함)
- [testing-library-guide.md](./testing-library-guide.md) — Functorium.Testing 라이브러리
