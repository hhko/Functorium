...
> DomainEvent Publisher는 Adapter 레이어에서 처리되며, `request.layer`는 `"adapter"`, `request.category`는 `"domain_event.publisher"`입니다.

**Application Usecase vs DomainEvent Publisher 필드 비교:**

| Field | Application Usecase | DomainEvent Publisher | 설명 |
|-------|---------------------|----------------------|------|
| `request.layer` | `"application"` | `"adapter"` | 레이어 구분 |
...
|-------|----------------------|-------------------|------|
| `request.layer` | `"adapter"` | `"application"` | 레이어 구분 |
