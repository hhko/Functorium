...
각 유효성 검사 조건(`NotNull`, `ThenNotEmpty`, `ThenMaxLength`, `ThenMatches`)은 실패 시 조건별로 세분화된 에러 코드를 자동 생성합니다.
예를 들어 `null` 입력은 `"DomainErrors.Email.Null"`, 빈 문자열은 `"DomainErrors.Email.Empty"`, 길이 초과는 `"DomainErrors.Email.TooLong"`, 정규식 불일치는 `"DomainErrors.Email.InvalidFormat"`이 됩니다.
복합 Value Object에서는 Apply 패턴으로 여러 필드를 병렬 검증하여, 복수 조건 실패 시 모든 에러를 한꺼번에 수집합니다.
