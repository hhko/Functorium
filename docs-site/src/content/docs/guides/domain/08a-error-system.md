...
- [트러블슈팅](#트러블슈팅)
- [FAQ](#faq)
- [부록: ErrorCodeFactory API](#부록-errorcodeFactory-api)
- [참고 문서](#참고-문서)
...
모든 `ErrorCodeExpected` 변형과 `ErrorCodeExceptional`은 **internal record**이며 `IHasErrorCode` 인터페이스를 구현합니다.
...
`ErrorCodeFactory`는 `Abstractions/Errors/ErrorCodeFactory.cs`에 위치한 **정적 클래스**입니다.
