using Cqrs10.Demo;
using Cqrs10.Demo.Infrastructure;
using Cqrs10.Demo.Usecases;
...
// Command 실행: 사용자 생성
FinResponse<CreateUserCommand.Response> createResult1 = await mediator.Send(
    new CreateUserCommand.Request("Alice", "alice@example.com"));

PrintResult("Create User (Alice)", createResult1);

FinResponse<CreateUserCommand.Response> createResult2 = await mediator.Send(
    new CreateUserCommand.Request("Bob", "bob@example.com"));

PrintResult("Create User (Bob)", createResult2);

// 중복 이메일로 생성 시도
FinResponse<CreateUserCommand.Response> createResult3 = await mediator.Send(
...
// Query 실행: 모든 사용자 조회
FinResponse<GetAllUsersQuery.Response> allUsersResult = await mediator.Send(
...
if (createResult1.IsSucc)
{
    ...
    FinResponse<GetUserByIdQuery.Response> userResult = await mediator.Send(
