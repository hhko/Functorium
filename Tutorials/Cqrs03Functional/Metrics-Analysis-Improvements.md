...
- IObservablePort Metrics: `adapter.{category}.op.request` (점 구분, 소문자, `.op` 포함)

**현재 상태**:
```
Usecase:   application.usecase.command.requests
IObservablePort:  adapter.repository.op.request
```

**개선 방안**:
- 메트릭 이름 형식을 통일: `application.usecase.{cqrs}.requests` vs `application.adapter.{category}.requests`
- 또는 IObservablePort의 `.op` 접미사 제거 또는 Usecase에도 추가
...
### 7. Usecase와 IObservablePort의 태그 차이
...
- IObservablePort: RequestCategory별 Meter 분리 (`{ServiceNamespace}.Adapter.{RequestCategory}`)

**현재 상태**:
```
Usecase Meter:   Cqrs03Functional.Demo.Application (1개)
IObservablePort Meter:  Cqrs03Functional.Demo.Adapter.Repository (N개)
...
**문제점**:
- IObservablePort Metrics는 첫 요청 시점에 초기화됨
...
1. **메트릭 이름 통일**: Usecase/IObservablePort 간 메트릭 이름 패턴 통일
