...
이 문서는 `Cqrs03Functional.Demo` 애플리케이션에서 수집되는 Trace(분산 추적)를 분석하여, **Usecase**와 **IObservablePort** 인터페이스 중심의 두 가지 Trace 범주로 구분하고 각각의 Activity 이름, 태그, 상태를 정리합니다.

## 목차

1. [Trace 범주 개요](#trace-범주-개요)
2. [Usecase Trace 형식](#usecase-trace-형식)
3. [IObservablePort Trace 형식](#iport-trace-형식)
...
### 2. IObservablePort Trace
- **생성 위치**: 소스 생성기로 자동 생성된 `*Pipeline` 클래스에서 `IObservablePortTrace` 호출
...
## IObservablePort Trace 형식

IObservablePort Trace는 소스 생성기(`ObservablePortGenerator`)에 의해 자동 생성된 Pipeline 클래스에서 `IObservablePortTrace` 인터페이스를 통해 수집됩니다. `[GenerateObservablePort]` 애트리뷰트가 적용된 클래스에 대해 자동으로 Trace 수집이 활성화됩니다.
...
#### 생성 시점
- 요청이 들어올 때 `_portTrace.Request(parentContext, requestCategory, requestHandler, requestHandlerMethod, startTime)` 호출
...
**참고**: IObservablePort Trace는 Usecase와 달리 에러 상세 정보를 태그로 기록하지 않습니다. 에러 정보는 Log를 통해 확인할 수 있습니다.

#### 설정 시점
- 응답 처리 완료 후 `_portTrace.ResponseSuccess()` 또는 `_portTrace.ResponseFailure()` 호출
...
Usecase Activity (Application Usecase.Command CreateProductCommand.Handle)
  └── IObservablePort Activity (Adapter Repository InMemoryProductRepository.Create)
        └── IObservablePort Activity (Adapter Repository InMemoryProductRepository.GetById)
```

**참고**: IObservablePort Activity는 Usecase Activity의 자식으로 생성되며, 여러 IObservablePort 호출이 중첩될 수 있습니다.
...
- **IObservablePort**: `Functorium.*` 패턴으로 등록 (Usecase와 동일한 ActivitySource 사용)

**참고**: 
- Usecase와 IObservablePort 모두 동일한 ActivitySource(`{ServiceName}`)를 사용합니다.
...
- **IObservablePort Trace 구현**: `Src/Functorium/Adapters/Observabilities/Tracing/AdapterTrace.cs`
