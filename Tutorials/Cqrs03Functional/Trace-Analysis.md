...
이 문서는 `Cqrs03Functional.Demo` 애플리케이션에서 수집되는 Trace(분산 추적)를 분석하여, **Usecase**와 **IPort** 인터페이스 중심의 두 가지 Trace 범주로 구분하고 각각의 Activity 이름, 태그, 상태를 정리합니다.

## 목차

1. [Trace 범주 개요](#trace-범주-개요)
2. [Usecase Trace 형식](#usecase-trace-형식)
3. [IPort Trace 형식](#iport-trace-형식)
...
### 2. IPort Trace
- **생성 위치**: 소스 생성기로 자동 생성된 `*Pipeline` 클래스에서 `IPortTrace` 호출
...
**참고**: IPort Trace는 Usecase와 달리 에러 상세 정보를 태그로 기록하지 않습니다. 에러 정보는 Log를 통해 확인할 수 있습니다.
...
Usecase Activity (Application Usecase.Command CreateProductCommand.Handle)
  └── IPort Activity (Adapter Repository InMemoryProductRepository.Create)
        └── IPort Activity (Adapter Repository InMemoryProductRepository.GetById)
```

**참고**: IPort Activity는 Usecase Activity의 자식으로 생성되며, 여러 IPort 호출이 중첩될 수 있습니다.
...
- **IPort**: `Functorium.*` 패턴으로 등록 (Usecase와 동일한 ActivitySource 사용)

**참고**: 
- Usecase와 IPort 모두 동일한 ActivitySource(`{ServiceName}`)를 사용합니다.
