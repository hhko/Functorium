...
이 문서는 `Cqrs03Functional.Demo` 애플리케이션 실행 시 출력되는 로그를 분석하여, **Usecase**와 **IObservablePort** 인터페이스 중심의 두 가지 로그 범주로 구분하고 각각의 로그 필드(JSON)와 메시지 템플릿을 정리합니다.

## 목차

1. [로그 범주 개요](#로그-범주-개요)
2. [Usecase 로그 형식](#usecase-로그-형식)
3. [IObservablePort 로그 형식](#iport-로그-형식)
4. [로그 필드 매핑](#로그-필드-매핑)
...
#### 메시지 템플릿
```
{RequestLayer} {RequestCategory}.{RequestCategoryType} {RequestHandler}.{RequestHandlerMethod} {@Request:Request} requesting
```
...
| `RequestCategoryType` | string | CQRS 타입 ("Command", "Query", "Unknown") | "Command" |
...
#### 메시지 템플릿
```
{RequestLayer} {RequestCategory}.{RequestCategoryType} {RequestHandler}.{RequestHandlerMethod} {@Response:Response} responded {Status} in {Elapsed:0.0000} ms
```
...
| `RequestLayer` | string | 요청 계층 (고정값: "Application") | "Application" |
| `RequestCategory` | string | 요청 카테고리 (고정값: "Usecase") | "Usecase" |
| `RequestCategoryType` | string | CQRS 타입 | "Command" |
| `RequestHandler` | string | Handler 클래스 이름 | "CreateProductCommand" |
...
#### 메시지 템플릿
```
{RequestLayer} {RequestCategory}.{RequestCategoryType} {RequestHandler}.{RequestHandlerMethod} responded {Status} in {Elapsed:0.0000} ms with {@Error:Error}
```
...
## IObservablePort 로그 형식

IObservablePort 로그는 소스 생성기(`ObservablePortGenerator`)에 의해 자동 생성된 Pipeline 클래스에서 출력됩니다. `[GenerateObservablePort]` 애트리뷰트가 적용된 클래스에 대해 자동으로 Pipeline이 생성됩니다.

**중요**: IObservablePort 로그는 **MinimumLevel 설정에 따라 출력 형식이 달라집니다**:
...
| `RequestCategory` | string | 요청 카테고리 (IObservablePort.RequestCategory 속성 값) | "Repository" |
...
| `RequestCategoryType` | `RequestCategoryType` | CQRS 타입 ("Command", "Query") |
...
### IObservablePort 로그 필드 매핑

| 로그 필드 | 설명 |
|-----------|------|
| `RequestLayer` | 요청 계층 ("Adapter") |
| `RequestCategory` | 요청 카테고리 (IObservablePort.RequestCategory 속성 값) |
| `RequestHandler` | Handler 클래스 이름 |
...
3. **소스 생성기 기반**: `[GenerateObservablePort]` 애트리뷰트로 자동 생성
...
| **IObservablePort** | Debug / Information* | Debug / Information* | Warning | Error |

\* **IObservablePort 로그 레벨 동작**:
- **Debug 레벨**: 파라미터와 반환값을 포함한 상세 로그 출력
