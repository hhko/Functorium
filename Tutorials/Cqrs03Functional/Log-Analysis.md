...
이 문서는 `Cqrs03Functional.Demo` 애플리케이션 실행 시 출력되는 로그를 분석하여, **Usecase**와 **IPort** 인터페이스 중심의 두 가지 로그 범주로 구분하고 각각의 로그 필드(JSON)와 메시지 템플릿을 정리합니다.

## 목차

1. [로그 범주 개요](#로그-범주-개요)
2. [Usecase 로그 형식](#usecase-로그-형식)
3. [IPort 로그 형식](#iport-로그-형식)
4. [로그 필드 매핑](#로그-필드-매핑)
...
IPort 로그는 소스 생성기(`PortObservableGenerator`)에 의해 자동 생성된 Pipeline 클래스에서 출력됩니다. `[GeneratePortObservable]` 애트리뷰트가 적용된 클래스에 대해 자동으로 Pipeline이 생성됩니다.

**중요**: IPort 로그는 **MinimumLevel 설정에 따라 출력 형식이 달라집니다**:
