...
이 문서는 `Cqrs03Functional.Demo` 애플리케이션에서 수집되는 Metrics를 분석하여, **Usecase**와 **IPort** 인터페이스 중심의 두 가지 Metrics 범주로 구분하고 각각의 메트릭 이름, 타입, 단위, 태그를 정리합니다.

## 목차

1. [Metrics 범주 개요](#metrics-범주-개요)
2. [Usecase Metrics 형식](#usecase-metrics-형식)
3. [IPort Metrics 형식](#iport-metrics-형식)
...
### 2. IPort Metrics
- **생성 위치**: 소스 생성기로 자동 생성된 `*Pipeline` 클래스에서 `IPortMetric` 호출
...
| `request.category` | RequestCategory 값 | 요청 카테고리 (IPort.RequestCategory 속성 값) | "Repository", "Db" |
