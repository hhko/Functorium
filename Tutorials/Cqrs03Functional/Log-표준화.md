...
- **대상**: `IPort` 인터페이스를 구현한 Repository 등의 Adapter
...
Adapter 레이어 로그는 소스 생성기(`PortObservableGenerator`)에 의해 자동 생성된 Pipeline 클래스에서 출력됩니다. `[GeneratePortObservable]` 애트리뷰트가 적용된 클래스에 대해 자동으로 Pipeline이 생성됩니다.
...
| `RequestCategory` | string | 요청 카테고리 (IPort.RequestCategory 속성 값) | "Repository" |
...
| `RequestCategory` | 요청 카테고리 (IPort.RequestCategory 속성 값) |
| `RequestHandler` | Handler 클래스 이름 |
...
3. **소스 생성기 기반**: `[GeneratePortObservable]` 애트리뷰트로 자동 생성
...
| **IPort** | Debug / Information* | Debug / Information* | Warning | Error |
