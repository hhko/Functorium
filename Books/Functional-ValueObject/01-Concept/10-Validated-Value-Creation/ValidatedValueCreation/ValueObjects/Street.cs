...
public sealed class Street : IEquatable<Street>
{
    ...
    public static Fin<Street> Create(string value) =>
        Validate(value)
            .Map(validValue => new Street(validValue))
            .ToFin();

    /// <summary>
    /// 이미 검증된 값으로부터 Street 인스턴스를 생성하는 internal 메서드
    ...
    /// <summary>
    /// Street을 string로 명시적 변환
    /// </summary>
    /// <param name="value">변환할 값</param>
    public static explicit operator string(Street street) =>
        street._value;
}
