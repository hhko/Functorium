...
public sealed class ClassValidator
{
    ...
    /// <summary>
    /// ClassValidator 인스턴스를 생성합니다.
    /// 
    /// 새로운 검증 세션을 시작할 때 사용됩니다.
    /// 각 검증 세션은 독립적인 failures 리스트를 가지며,
    /// 검증 결과는 ValidationResult로 반환됩니다.
    /// </summary>
    /// <param name="architecture">ArchUnitNET 아키텍처 인스턴스</param>
    /// <param name="targetClass">검증할 대상 클래스</param>
    ...
    /// <summary>
    /// 중첩 검증을 위한 ClassValidator 인스턴스를 생성합니다.
    /// 
    ...
    /// <summary>
    /// 중첩 클래스가 존재하는지 검증하고, 존재하면 추가 검증을 수행합니다.
    /// 
    /// 이 메서드는 지정된 이름의 중첩 클래스가 반드시 존재해야 하는 경우에 사용됩니다.
    /// 중첩 클래스가 없으면 검증 실패로 처리됩니다.
    /// 
    /// 중첩 클래스의 FullName은 "외부클래스명+중첩클래스명" 형식으로 구성됩니다.
    /// 
    /// 사용 예시:
    /// <code>
    /// @class.RequireNestedClass("Error", nested => nested
    ///     .RequirePublic()
    ///     .RequireSealed());
    /// </code>
    /// </summary>
    /// <param name="nestedClassName">검증할 중첩 클래스 이름</param>
    /// <param name="nestedClassValidation">중첩 클래스에 대한 추가 검증 로직 (선택사항)</param>
    ...
    /// <summary>
    /// 중첩 클래스가 존재할 때만 검증을 수행하는 메서드입니다.
    /// 
    /// 이 메서드는 중첩 클래스가 선택적으로 존재할 수 있는 경우에 사용됩니다.
    /// 중첩 클래스가 없으면 검증을 건너뛰고 성공으로 처리하며,
    /// 존재하면 추가 검증을 수행합니다.
    /// 
    /// 사용 예시:
    /// <code>
    /// @class.RequireNestedClassIfExists("Error", nested => nested
    ///     .RequirePublic()
    ///     .RequireSealed());
    /// </code>
    /// </summary>
    /// <param name="nestedClassName">검증할 중첩 클래스 이름</param>
    /// <param name="nestedClassValidation">중첩 클래스에 대한 검증 로직 (선택사항)</param>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스가 public 가시성을 가져야 함을 검증합니다.
    /// 
    /// ValueObject나 도메인 엔티티 등 외부에서 접근 가능해야 하는
    /// 클래스들의 가시성을 검증할 때 사용됩니다.
    /// </summary>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스가 internal 가시성을 가져야 함을 검증합니다.
    /// 
    /// 같은 어셈블리 내에서만 접근 가능해야 하는
    /// 내부 구현 클래스들의 가시성을 검증할 때 사용됩니다.
    /// </summary>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스가 sealed(봉인) 클래스여야 함을 검증합니다.
    /// 
    /// ValueObject나 유틸리티 클래스 등 상속을 허용하지 않아야 하는
    /// 클래스들의 봉인 여부를 검증할 때 사용됩니다.
    /// </summary>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스가 지정된 인터페이스를 구현해야 함을 검증합니다.
    /// 
    /// 인터페이스 구현 여부를 검증할 때 사용됩니다.
    /// 제네릭 인터페이스의 경우도 지원합니다.
    /// 
    /// 사용 예시:
    /// <code>
    /// @class.RequireImplements(typeof(IValueObject))
    /// @class.RequireImplements(typeof(IComparable&lt;&gt;))
    /// </code>
    /// </summary>
    /// <param name="interfaceType">구현해야 하는 인터페이스 타입</param>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스가 지정된 제네릭 인터페이스를 구현해야 함을 검증합니다.
    /// 
    /// 제네릭 인터페이스의 경우 타입 매개변수가 다를 수 있으므로
    /// 인터페이스 이름만으로 검증합니다.
    /// 
    /// 사용 예시:
    /// <code>
    /// @class.RequireImplementsGenericInterface("IComparable")
    /// @class.RequireImplementsGenericInterface("IEquatable")
    /// </code>
    /// </summary>
    /// <param name="genericInterfaceName">구현해야 하는 제네릭 인터페이스 이름</param>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스가 지정된 기본 클래스를 상속해야 함을 검증합니다.
    /// 
    /// 상속 관계를 검증할 때 사용됩니다.
    /// 
    /// 사용 예시:
    /// <code>
    /// @class.RequireInherits(typeof(ValueObject))
    /// @class.RequireInherits(typeof(Entity))
    /// </code>
    /// </summary>
    /// <param name="baseType">상속해야 하는 기본 클래스 타입</param>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스가 매개변수가 없는 private 생성자를 가져야 함을 검증합니다.
    /// 
    /// ValueObject나 Entity 등에서 매개변수 없는 생성자가
    /// private이어야 하는 경우에 사용됩니다.
    /// </summary>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    public ClassValidator RequireAnyPrivateParameterlessConstructor()
    {
    ...
    /// <summary>
    /// 클래스의 모든 생성자가 private이어야 함을 검증합니다.
    /// 
    /// ValueObject 등에서 외부에서 직접 인스턴스 생성을 방지하고
    /// 팩토리 메서드를 통해서만 생성하도록 강제할 때 사용됩니다.
    /// </summary>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 지정된 이름의 메서드가 존재하는지 검증하고, 존재하면 추가 검증을 수행합니다.
    /// 
    /// 이 메서드는 특정 이름의 메서드가 반드시 존재해야 하는 경우에 사용됩니다.
    /// 메서드가 없으면 검증 실패로 처리되며, 존재하면 MethodValidator를 통해
    /// 세부적인 검증을 수행할 수 있습니다.
    /// 
    /// 메서드 이름은 "메서드명(" 형식으로 검색하므로 오버로드된 메서드들도
    /// 모두 검증 대상에 포함됩니다.
    /// 
    /// 사용 예시:
    /// <code>
    ...
    /// </code>
    /// </summary>
    /// <param name="methodName">검증할 메서드 이름</param>
    /// <param name="methodValidation">메서드에 대한 세부 검증 로직</param>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    /// <summary>
    /// 클래스의 모든 메서드에 대해 검증을 수행합니다.
    /// 
    /// 이 메서드는 클래스에 정의된 모든 메서드에 대해 동일한 검증 규칙을
    /// 적용할 때 사용됩니다. 예를 들어, 모든 public 메서드가 static이어야 한다거나,
    /// 모든 메서드가 특정 반환 타입을 가져야 한다는 규칙을 검증할 때 유용합니다.
    /// 
    /// 사용 예시:
    /// <code>
    /// @class.RequireAllMethods(method => method
    ///     .RequireVisibility(Visibility.Public)
    ///     .RequireStatic());
    /// </code>
    /// </summary>
    /// <param name="methodValidation">모든 메서드에 적용할 검증 로직</param>
    /// <returns>ClassValidator 인스턴스 (메서드 체이닝용)</returns>
    ...
    public ClassValidator RequireImmutable()
    {
        ...
        failures.AddRange(ValidateConstructors());
        failures.AddRange(ValidateProperties());
        failures.AddRange(ValidateFields());
        failures.AddRange(ValidateMutableCollections(nonStaticMembers));
        failures.AddRange(ValidateStateChangingMethods());
    ...
    private List<string> ValidateWritability(List<IMember> nonStaticMembers)
    {
    ...
    /// </summary>
    /// <returns>검증 실패 시 오류 메시지 목록, 성공 시 빈 목록</returns>
    private List<string> ValidateConstructors()
    {
    ...
    /// <param name="nonStaticMembers">검증할 non-static 멤버 목록</param>
    /// <returns>검증 실패 시 오류 메시지 목록, 성공 시 빈 목록</returns>
    private List<string> ValidateMutableCollections(List<IMember> nonStaticMembers)
    {
    ...
    /// </summary>
    /// <returns>검증 실패 시 오류 메시지 목록, 성공 시 빈 목록</returns>
    private List<string> ValidateStateChangingMethods()
    {
