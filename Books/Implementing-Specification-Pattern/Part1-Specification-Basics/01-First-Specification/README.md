# Part 1 - Chapter 1: 첫 번째 Specification

> **Part 1: Specification 기초** | [← 목차로](../../../README.md) | [다음: 2장 조합 →](../02-Composition/README.md)
...
### 실습을 통해 확인할 내용
- **InStockSpec**: 재고가 0보다 큰 상품 필터링
- **PriceRangeSpec**: 최솟값/최댓값 범위 내 상품 필터링
...
├── Product.cs                          # 도메인 모델
├── Specifications/
│   ├── InStockSpec.cs                  # 재고 확인 Specification
│   └── PriceRangeSpec.cs               # 가격 범위 Specification
└── FirstSpecification.csproj

FirstSpecification.Tests.Unit/
├── InStockSpecTests.cs                 # 재고 경계값 테스트
├── PriceRangeSpecTests.cs              # 가격 범위 경계값 테스트
├── Using.cs
├── xunit.runner.json
...
**A**: 구체적인 Specification 클래스(`InStockSpec`, `PriceRangeSpec`)는 더 이상 상속할 필요가 없는 최종 구현입니다. `sealed`를 붙이면 의도하지 않은 상속을 방지하고, JIT 컴파일러가 가상 호출을 최적화할 수 있습니다.

### Q3: Specification에 상태(생성자 매개변수)를 전달해도 되나요?
**A**: 네, `PriceRangeSpec(min, max)`처럼 생성 시점에 조건 매개변수를 받는 것은 자연스러운 패턴입니다. Specification은 생성 후 불변(immutable)이어야 하며, `IsSatisfiedBy` 호출 시 외부 상태에 의존하지 않아야 합니다.

### Q4: 경계값 테스트가 왜 중요한가요?
**A**: `PriceRangeSpec(100, 500)`에서 가격이 정확히 100이거나 500일 때의 동작은 `>=`인지 `>`인지에 따라 달라집니다. 경계값 테스트는 이런 미묘한 차이를 명확히 검증하여 Specification의 정확한 의미를 보장합니다.
