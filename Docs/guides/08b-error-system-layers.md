...
이 문서는 Domain/Application/Adapter 레이어별 에러 정의, Custom 에러, 테스트 패턴, 체크리스트를 다룹니다. 에러 처리의 기본 원칙과 네이밍 규칙은 [08a-error-system.md](./08a-error-system.md)을 참고하세요.
...
1. 에러가 발생하는 레이어 결정 (Domain / Application / Adapter)
2. 표준 에러 타입 선택 또는 Custom sealed record 정의
3. 레이어 팩토리로 에러 생성 (`DomainError.For`, `ApplicationError.For`, `AdapterError.For`)
...
### Q3. 범용 어설션과 레이어별 어설션 중 어느 것을 사용해야 하나요?
