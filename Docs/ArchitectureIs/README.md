...
| 연산 | Application | Mediator 패턴(CQRS) | `IRequest/IResponse` 인터페이스 (`ICommandRequest`, `IQueryRequest`) |
| 출력(Response) | Adapter | Strategy 패턴 | `IAdapter` 인터페이스를 통한 외부 시스템 연동 |
| 비즈니스 단위 | Domain | - | 비즈니스 핵심 로직과 엔티티 |

### 테스트 자동화
![](./InternalArchitecture_Test.png)
