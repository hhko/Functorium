...
| 2 | **이중 인터페이스 복잡성** | `IResponse`를 감싸는 `IFinResponse` 인터페이스가 추가로 필요하여 관리 복잡성 증가 |
| 3 | **리플렉션 의존** | Pipeline에서 성공/실패 확인 및 실패 응답 생성 시 리플렉션 필요 |

특히 3번 문제인 리플렉션은 성능 저하와 타입 안전성 문제를 야기합니다.
...
Pipeline이 응답을 인식하려면 반드시 `IFinResponse` 인터페이스를 정의해야 했습니다. `Fin<T>`는 외부 라이브러리 타입이므로 직접 수정할 수 없어서 래퍼 인터페이스가 필요했습니다.

```csharp
// Pipeline에서 Fin<T>를 직접 사용 불가
where TResponse : Fin<T>  // 불가능 - Fin<T>는 sealed struct
```

#### 문제 2: 이중 인터페이스 복잡성

`IResponse`(비즈니스 응답)와 `IFinResponse`(성공/실패 래퍼) 두 개의 인터페이스가 필요했습니다:
...
이로 인해:
- 두 인터페이스 간의 관계 관리 복잡
- Response 정의 시 두 인터페이스 모두 고려 필요
- 코드 가독성 저하
...
| 문제 | Before | After |
|------|--------|-------|
| **1. 인터페이스 강제** | `IFinResponse` 래퍼 필요 | `IResponse<T>` 단일 인터페이스 |
| **2. 이중 인터페이스** | `IResponse` + `IFinResponse` 2개 | `IResponse<T>` 1개로 통합 |
| **3. 리플렉션 의존** | 3곳에서 리플렉션 | **0곳** - 완전 제거 |
...
```
Before (이중 구조):                    After (단일 구조):
┌─────────────────────┐               ┌─────────────────────┐
│   Fin<TResponse>    │               │   TResponse         │
│  ┌───────────────┐  │               │  ┌───────────────┐  │
│  │ IFinResponse  │  │      →        │  │ IResponse<T>  │  │
│  │ ┌───────────┐ │  │               │  │ - IsSuccess   │  │
│  │ │ IResponse │ │  │               │  │ - Error       │  │
│  │ └───────────┘ │  │               │  │ - CreateFail()│  │
│  └───────────────┘  │               │  └───────────────┘  │
...
`ResponseBase<TSelf>`의 `CreateFail` 메서드 구현 때문입니다:

```csharp
public abstract record ResponseBase<TSelf> : IResponse<TSelf>
    where TSelf : ResponseBase<TSelf>, new()  // ← new() 제약
{
    public static TSelf CreateFail(Error error)
        => new TSelf() { IsSuccess = false, Error = error };  // ← new TSelf() 호출
}
```

**동작 원리:**
1. Pipeline에서 실패 응답 생성 시 `TResponse.CreateFail(error)` 호출
2. `CreateFail`은 `new TSelf()`로 빈 인스턴스 생성
3. `with` 대신 init 속성으로 `IsSuccess = false`, `Error = error` 설정
4. `new()` 제약이 없으면 `new TSelf()` 호출 불가 → 리플렉션 필요

**positional record의 특성:**
```csharp
public sealed record class Response(Guid ProductId, ...) : ResponseBase<Response>
```
이 문법은 `Response(Guid ProductId, ...)`라는 **주 생성자만** 생성합니다. C#은 모든 파라미터에 기본값이 있어도 이를 파라미터 없는 생성자로 인식하지 않습니다. 따라서 명시적으로 추가해야 합니다:
```csharp
public Response() : this(Guid.Empty, string.Empty, ..., default) { }
```
