...
| 어조 | 명령형으로 작성 | ✓ `추가` ✗ `추가한다` |
| 마침표 | 사용 금지 | ✓ `feat: 로그인 추가` ✗ `feat: 로그인 추가.` |
...
| 원칙 | 설명 | 예시 |
|------|------|------|
| 원자적 커밋 | 하나의 커밋은 하나의 논리적 변경만 포함 | ✓ 기능 추가 1개<br>✗ 기능 추가 + 버그 수정 |
...
- [Git 명령어 가이드](./Git-Commands.md) - Git 명령어 사용법
