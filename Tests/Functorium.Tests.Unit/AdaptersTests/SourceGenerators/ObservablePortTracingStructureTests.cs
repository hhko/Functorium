...
/// <summary>
/// SourceGenerator로 생성된 Adapter Observable의 Tracing 태그 구조 검증 테스트.
/// 런타임에서 실제 Observable을 실행하고 Activity 태그를 스냅샷으로 검증합니다.
/// </summary>
/// <remarks>
/// <para>
/// 이 테스트는 생성된 Observable 코드의 Tracing 태그 구조가 실수로 변경되는 것을 방지합니다.
/// </para>
/// <para>
/// Activity 태그 구조 비교표:
/// </para>
...
public sealed class ObservablePortTracingStructureTests : IDisposable
{
    ...
    public ObservablePortTracingStructureTests()
    {
    ...
    {
        // Arrange
        var logger = _logContext.CreateLogger<TestObservabilityAdapterObservable>();
    ...
    public async Task Snapshot_SuccessTags()
    {
        ...
        await Verify(tags)
            .UseDirectory("Snapshots/ObservablePortTracingStructure")
