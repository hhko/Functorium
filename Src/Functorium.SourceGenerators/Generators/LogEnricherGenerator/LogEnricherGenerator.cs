        ...
        AttachDebugger: false)
{
    ...
    private static string GenerateEnricherSource(LogEnricherInfo info, StringBuilder sb)
    {
        ...
        sb.AppendLine($"    : global::Functorium.Adapters.Observabilities.Pipelines.IUsecaseLogEnricher<{requestFullType}, {finResponseType}>");
        sb.AppendLine("{");
        ...
        if (requestCount > 0)
        {
            ...
            foreach (var prop in info.RequestProperties)
            {
                ...
                {
                    sb.AppendLine($"        disposables.Add(global::Serilog.Context.LogContext.PushProperty(\"{prop.CtxFieldName}\", {prop.CountExpression}));");
                }
                else
                {
                    sb.AppendLine($"        disposables.Add(global::Serilog.Context.LogContext.PushProperty(\"{prop.CtxFieldName}\", request.{prop.PropertyName}));");
                }
            }
        ...
        if (responseCount > 0)
        {
            ...
            foreach (var prop in info.ResponseProperties)
            {
                ...
                {
                    sb.AppendLine($"            disposables.Add(global::Serilog.Context.LogContext.PushProperty(\"{prop.CtxFieldName}\", r.{prop.PropertyName}));");
                }
            }
            sb.AppendLine("        }");
        }
