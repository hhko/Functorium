        ...
        AttachDebugger: false)  // ⚠️ 디버깅 필요 시 true로 변경 (자세한 내용: DEBUGGING_SOURCE_GENERATOR.md 참조)
{
    ...
    private static string GeneratePipelineClassSource(PipelineClassInfo classInfo, StringBuilder sb)
    {
        sb.Append(Header)
            ...
            .AppendLine("using Functorium.Adapters.Observabilities.Abstractions.Utilities;")
    ...
    private static void GenerateHelperMethods(StringBuilder sb, PipelineClassInfo classInfo)
    {
        sb.AppendLine("        private static global::LanguageExt.IO<A> FinTToIO<A>(global::LanguageExt.FinT<global::LanguageExt.IO, A> finT) =>")
            ...
            .AppendLine("                Fail: error => global::LanguageExt.IO<A>.Fail(error)")
            .AppendLine("            )).Flatten();")
            .AppendLine()
            ...
            .AppendLine("                        from _setContext in global::LanguageExt.IO.pure(global::Functorium.Applications.Observabilities.UsecaseActivityContext.SetCurrentUnit(_parentContext))")
            ...
            .AppendLine("                    {")
            .AppendLine("                        global::Functorium.Applications.Observabilities.UsecaseActivityContext.SetCurrent(null);")
            ...
            .AppendLine("            return global::LanguageExt.IO<A>.Fail(error);")
            .AppendLine("        }")
            .AppendLine()
    ...
    private static void GenerateMethod(StringBuilder sb, PipelineClassInfo classInfo, MethodInfo method)
    {
        ...
        sb.AppendLine("        ) =>")
            .AppendLine($"            {method.ReturnType}.LiftIO(")
            ...
            .AppendLine($"                    requestLog: () => RequestLog_{classInfo.ClassName}_{method.Name}(RequestHandler, nameof({method.Name}), {string.Join(", ", method.Parameters.Select(p => p.Name))}),")
        ...
        sb.AppendLine($"        private global::LanguageExt.IO<global::LanguageExt.Unit> RequestLog_{classInfo.ClassName}_{method.Name}(")
            .AppendLine("            string requestHandler,")
            .AppendLine("            string requestHandlerMethod,")
            .AppendLine($"            {string.Join(", ", method.Parameters.Select(p => $"{p.Type} {p.Name}"))}) =>")
            .AppendLine("            global::LanguageExt.IO.lift(() =>")
            .AppendLine("            {")
            ...
            .AppendLine("                        requestHandler,")
            .AppendLine("                        requestHandlerMethod,")
            .AppendLine($"                        {string.Join(", ", method.Parameters.Select(p => p.Name))});")
            .AppendLine("                }")
