        ...
        AttachDebugger: false)  // ⚠️ 디버깅 필요 시 true로 변경 (자세한 내용: DEBUGGING_SOURCE_GENERATOR.md 참조)
{
    ...
    private static string GeneratePipelineClassSource(PipelineClassInfo classInfo, StringBuilder sb)
    {
        sb.Append(Header)
            ...
            .AppendLine("    private readonly Counter<long> _successCounter;")
            .AppendLine("    private readonly Counter<long> _failureCounter;")
        ...
        sb.AppendLine()
            ...
            .AppendLine("        _successCounter = meter.CreateCounter<long>($\"adapter.{categoryLower}.responses.success\", \"{response}\", \"Total number of successful responses\");")
            .AppendLine("        _failureCounter = meter.CreateCounter<long>($\"adapter.{categoryLower}.responses.failure\", \"{response}\", \"Total number of failed responses\");")
    ...
    private static void GenerateHelperMethods(StringBuilder sb, PipelineClassInfo classInfo)
    {
        sb.AppendLine("    private static global::LanguageExt.IO<A> FinTToIO<A>(global::LanguageExt.FinT<global::LanguageExt.IO, A> finT) =>")
            ...
            .AppendLine("        };")
            .AppendLine("        _successCounter.Add(1, metricTags);")
            ...
            .AppendLine("            { ObservabilityNaming.CustomAttributes.RequestHandlerMethod, \"Method\" }")
            .AppendLine("        };")
            .AppendLine("        _failureCounter.Add(1, metricTags);")
            .AppendLine("        _durationHistogram.Record(elapsed / 1000.0, metricTags); // ms to seconds")
            .AppendLine()
            .AppendLine("        activity?.SetTag(ObservabilityNaming.CustomAttributes.ResponseElapsed, elapsed);")
