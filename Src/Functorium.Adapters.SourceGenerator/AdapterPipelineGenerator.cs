        ...
        AttachDebugger: false)  // ⚠️ 디버깅 필요 시 true로 변경 (자세한 내용: DEBUGGING_SOURCE_GENERATOR.md 참조)
{
    ...
    private static void GenerateHelperMethods(StringBuilder sb, PipelineClassInfo classInfo)
    {
        sb.AppendLine("    private static global::LanguageExt.IO<A> FinTToIO<A>(global::LanguageExt.FinT<global::LanguageExt.IO, A> finT) =>")
            ...
            .AppendLine("                    RecordActivitySuccess(activity, elapsed);")
            .AppendLine("                    return global::LanguageExt.Unit.Default;")
            .AppendLine("                })")
            .AppendLine("                select result)")
            ...
            .AppendLine("        RecordActivityFailure(activity, error, elapsed);")
            ...
            .AppendLine("        });")
            .AppendLine()
            .AppendLine("    private void RecordActivitySuccess(Activity? activity, double elapsed)")
            .AppendLine("    {")
            .AppendLine("        TagList metricTags = new()")
            .AppendLine("        {")
            .AppendLine("            { ObservabilityNaming.CustomAttributes.RequestLayer, ObservabilityNaming.Layers.Adapter },")
            .AppendLine($"            {{ ObservabilityNaming.CustomAttributes.RequestCategory, this.GetRequestCategoryPascalCase() }},")
            .AppendLine("            { ObservabilityNaming.CustomAttributes.RequestHandler, RequestHandler },")
            .AppendLine("            { ObservabilityNaming.CustomAttributes.RequestHandlerMethod, \"Method\" },")
            ...
            .AppendLine("    }")
            .AppendLine()
            .AppendLine("    private void RecordActivityFailure(Activity? activity, global::LanguageExt.Common.Error error, double elapsed)")
            .AppendLine("    {")
            ...
            .AppendLine("        activity?.SetTag(ObservabilityNaming.CustomAttributes.ErrorMessage, error.Message);")
            .AppendLine("        activity?.SetStatus(ActivityStatusCode.Error, error.Message);")
            .AppendLine("    }")
            .AppendLine()
            ...
            .AppendLine("            ),")
            .AppendLine("            // ErrorCodeExceptional - Exceptional 에러 (IHasErrorCode보다 먼저 매칭)")
            .AppendLine("            global::Functorium.Abstractions.ErrorCodeExceptional exceptional => (")
            .AppendLine("                ErrorType: ObservabilityNaming.ErrorTypes.Exceptional,")
            .AppendLine("                ErrorCode: exceptional.ErrorCode")
            .AppendLine("            ),")
            .AppendLine("            // IHasErrorCode - Expected 에러")
            .AppendLine("            global::Functorium.Abstractions.IHasErrorCode hasErrorCode => (")
            .AppendLine("                ErrorType: ObservabilityNaming.ErrorTypes.Expected,")
            ...
            .AppendLine()
            .AppendLine("    /// <summary>")
            .AppendLine("    /// ManyErrors에서 우선순위가 높은 에러 코드를 추출합니다.")
            .AppendLine("    /// Exceptional 에러가 Expected보다 우선합니다.")
            .AppendLine("    /// </summary>")
            .AppendLine("    private static string GetPrimaryErrorCode(global::LanguageExt.Common.ManyErrors many)")
            .AppendLine("    {")
            .AppendLine("        string? exceptionalCode = null;")
            .AppendLine("        string? expectedCode = null;")
            .AppendLine()
            .AppendLine("        foreach (var err in many.Errors)")
            .AppendLine("        {")
            .AppendLine("            if (err is global::Functorium.Abstractions.ErrorCodeExceptional exceptional)")
            .AppendLine("            {")
            .AppendLine("                exceptionalCode ??= exceptional.ErrorCode;")
            .AppendLine("            }")
            .AppendLine("            else if (err is global::Functorium.Abstractions.IHasErrorCode hasCode)")
            .AppendLine("            {")
            .AppendLine("                expectedCode ??= hasCode.ErrorCode;")
            .AppendLine("            }")
            .AppendLine("        }")
            .AppendLine()
            .AppendLine("        return exceptionalCode ?? expectedCode ?? many.GetType().Name;")
            .AppendLine("    }")
            .AppendLine();
    }
