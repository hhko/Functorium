...
/// </summary>
public sealed class SloConfiguration : IStartupOptionsLogger
{
    ...
    /// </summary>
    private SloTargets GetCqrsDefaults(string cqrsType)
    {
    ...
    /// </summary>
    private SloTargets MergeWithGlobalDefaults(SloTargets cqrsTargets)
    {
        return new SloTargets
        {
    ...
    public void LogConfiguration(ILogger logger)
    {
        ...
        LogSloTargets(logger, GlobalDefaults, labelWidth);
        logger.LogInformation("");

        // CQRS Defaults
        logger.LogInformation("  CQRS Defaults");
        logger.LogInformation("    Command:");
        LogSloTargets(logger, MergeWithGlobalDefaults(CqrsDefaults.Command), labelWidth, "      ");
        logger.LogInformation("    Query:");
        LogSloTargets(logger, MergeWithGlobalDefaults(CqrsDefaults.Query), labelWidth, "      ");
        logger.LogInformation("");

        // Handler Overrides
        if (HandlerOverrides.Count > 0)
        {
            logger.LogInformation("  Handler Overrides ({Count})", HandlerOverrides.Count);
            foreach (var (handler, targets) in HandlerOverrides)
            {
