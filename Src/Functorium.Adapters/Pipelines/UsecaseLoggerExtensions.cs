...
public static class UsecaseLoggerExtensions
{
        ...
        T? request)
    {
        ...
        logger.LogInformation(
            ...
            message: "{request.layer} {request.category.name}.{request.category.type} {request.handler.name}.{request.handler.method} requesting with {@request.message}",
            requestLayer,
            requestCategory,
            requestCategoryType,
            requestHandler,
        ...
        double responseElapsed)
    {
        ...
        logger.LogInformation(
            ...
            message: "{request.layer} {request.category.name}.{request.category.type} {request.handler.name}.{request.handler.method} responded {response.status} in {response.elapsed:0.0000} s with {@response.message}",
            requestLayer,
            requestCategory,
            requestCategoryType,
            requestHandler,
        ...
        Error error)
    {
        ...
        logger.LogWarning(
            ...
            message: "{request.layer} {request.category.name}.{request.category.type} {request.handler.name}.{request.handler.method} responded {response.status} in {response.elapsed:0.0000} s with {error.type}:{error.code} {@error}",
            requestLayer,
            requestCategory,
            requestCategoryType,
            requestHandler,
