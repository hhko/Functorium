...
public static class DomainEventHandlerLoggerExtensions
{
    ...
    /// <summary>
    /// 배치 도메인 이벤트 핸들러 요청 로그를 출력합니다.
    /// </summary>
    public static void LogDomainEventBatchHandlerRequest(
        this ILogger logger,
        string handlerName,
        string eventTypeName,
        int eventCount)
    {
        ...
        logger.LogInformation(
            eventId: ObservabilityNaming.EventIds.Application.ApplicationRequest,
            message: "{request.layer} {request.category.name}.{request.category.type} {request.handler.name}.{request.handler.method} {request.event.type} requesting with {request.event.count} events",
            ObservabilityNaming.Layers.Application,
            ObservabilityNaming.Categories.Usecase,
            ObservabilityNaming.CategoryTypes.Event,
            handlerName,
            ObservabilityNaming.Methods.HandleBatch,
            eventTypeName,
            eventCount);
    }

    /// <summary>
    /// 배치 도메인 이벤트 핸들러 응답 성공 로그를 출력합니다.
    /// </summary>
    public static void LogDomainEventBatchHandlerResponseSuccess(
        this ILogger logger,
        string handlerName,
        string eventTypeName,
        int eventCount,
        double elapsed)
    {
        ...
        logger.LogInformation(
            eventId: ObservabilityNaming.EventIds.Application.ApplicationResponseSuccess,
            message: "{request.layer} {request.category.name}.{request.category.type} {request.handler.name}.{request.handler.method} {request.event.type} responded {response.status} in {response.elapsed:0.0000} s with {request.event.count} events",
            ObservabilityNaming.Layers.Application,
            ObservabilityNaming.Categories.Usecase,
            ObservabilityNaming.CategoryTypes.Event,
            handlerName,
    ...
    /// <summary>
    /// 배치 도메인 이벤트 핸들러 응답 에러 로그를 출력합니다 (예외 포함).
    /// </summary>
    public static void LogDomainEventBatchHandlerResponseError(
        this ILogger logger,
        string handlerName,
        string eventTypeName,
        int eventCount,
        ...
        Exception exception)
    {
        ...
        logger.LogError(
            ObservabilityNaming.EventIds.Application.ApplicationResponseError,
            exception,
            "{request.layer} {request.category.name}.{request.category.type} {request.handler.name}.{request.handler.method} {request.event.type} responded {response.status} in {response.elapsed:0.0000} s with {request.event.count} events with {error.type}:{error.code}",
            ObservabilityNaming.Layers.Application,
            ObservabilityNaming.Categories.Usecase,
            ObservabilityNaming.CategoryTypes.Event,
            handlerName,
