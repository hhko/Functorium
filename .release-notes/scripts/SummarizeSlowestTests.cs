...
// Usage: dotnet SummarizeSlowestTests.cs [globPattern] [--threshold <seconds>]
// Default pattern: "**/TestResults/**/*.trx"
// Default threshold: 30 seconds (slow test threshold)
// Output: .analysis-output/test-summary.md
...
const string OutputFileName = "test-summary.md";
const string OutputDirName = ".analysis-output";
...
rootCommand.SetAction(async (result, cancellationToken) =>
{
    ...
    // .analysis-output 폴더에 파일 생성
    var outputDir = Path.Combine(scriptsDir, OutputDirName);
